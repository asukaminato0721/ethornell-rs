...
use crate::text_anim::normalize_message_text;
...
const MESSAGE_WINDOW_LAYER_ID: i32 = -20_010;
const MESSAGE_NAME_WINDOW_LAYER_ID: i32 = -20_011;
...
impl RuntimeTraceApi {
    pub(crate) fn observe_user_message(
        ...
        let Some(text) = stack.iter().rev().find_map(value_to_text_string) else {
            self.trace_graph("UserMessage without string payload");
            return;
        };
        if text.is_empty() {
            self.trace_graph("UserMessage empty");
            return;
        }
        let target = self.ensure_message_text_node();
        let display_text = wrap_text_to_state(&normalize_message_text(&text), &self.text_state);
        self.text_runtime.start_message(display_text, target);
        self.apply_text_to_node(target, String::new());
        tracing::info!(target, text = %normalize_message_text(&text), "UserMessage");
        self.trace_graph(format!(
            "message node #{target} start {:?}",
