...
const PERFORMANCE_PROFILING_PARAMETERS: &[NativeParameterSpec] = &[NativeParameterSpec {
    ...
    kind: "bool/i32",
    description:
        "Stored in the target profiling gate; nonzero also resets all accumulated counters.",
}];
...
const INPUT_DESCRIPTOR_LIST_PARAMETERS: &[NativeParameterSpec] = &[NativeParameterSpec {
    ...
    description:
        "The handler traverses descriptor identifiers and sums each target current-count field.",
}];
...
const TWO_PATH_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec {
        ...
        kind: "Shift-JIS path",
        description:
            "First BP argument; target passes the second popped string as the Win32 destination.",
    },
    NativeParameterSpec {
        name: "source",
        kind: "Shift-JIS path",
        description:
            "Second BP argument and first native pop; target passes it as the Win32 source.",
    },
];
const PATH_PARAMETER: &[NativeParameterSpec] = &[NativeParameterSpec {
    ...
    kind: "Shift-JIS path",
    description:
        "Native filesystem path resolved using the target process working-directory rules.",
}];
...
const SPECIAL_FOLDER_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "destination", kind: "BP char buffer", description: "Receives the selected folder path." },
    NativeParameterSpec { name: "mode", kind: "i32 enum 0..=5", description: "Selects Windows, desktop, programs, documents, common-program-files, or ProgramFilesDir." },
];
...
const DELETE_FILE_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "root", kind: "Shift-JIS path or null", description: "When non-null, target constructs root\\file; otherwise it prefixes the primary global root." },
    NativeParameterSpec { name: "file", kind: "Shift-JIS relative path", description: "File name deleted by DeleteFileA." },
];
...
const UNRECOVERED_SINGLE_PARAMETER: &[NativeParameterSpec] = &[NativeParameterSpec {
    ...
    description:
        "Target ABI proves one input, but neither its type nor meaning is currently proven.",
}];
...
const INPUT_SCOPE_PARAMETER: &[NativeParameterSpec] = &[NativeParameterSpec {
    ...
    description:
        "The target packs this value as `(scope << 16) | 0xFFFF` before registering or querying it.",
}];
...
const GRAPH92_OPEN_GLOBAL_MOVIE_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "archive", kind: "script string", description: "Optional archive/search namespace converted by sub_48DF50." },
    NativeParameterSpec { name: "resource", kind: "script string", description: "Movie resource converted by sub_48DF50 and forwarded to sub_48F270." },
    NativeParameterSpec { name: "argument_02", kind: "i32", description: "Consumed by sub_486B80 but not forwarded to sub_48F270; exact meaning remains unresolved." },
    NativeParameterSpec { name: "argument_03", kind: "i32", description: "Consumed by sub_486B80 but not forwarded to sub_48F270; exact meaning remains unresolved." },
    NativeParameterSpec { name: "width", kind: "positive i32", description: "Validated by sub_486B80 before the global movie is opened." },
    NativeParameterSpec { name: "height", kind: "positive i32", description: "Validated by sub_486B80 before the global movie is opened." },
];
...
const GRAPH90_REQUEST_REDRAW_PARAMETERS: &[NativeParameterSpec] = &[NativeParameterSpec {
    ...
    kind: "boolean",
    description:
        "Nonzero requests a full redraw; full redraw dominates an already-pending partial request.",
}];
...
const GRAPH90_SET_DISPLAY_OPTIONS_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        kind: "i32 <=256",
        description:
            "Second setting; target validates the popped mode and invalidates display objects.",
    },
];
...
const GRAPH90_BLIT_BITMAP_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "arg0", kind: "i32 / bitmap operand", description: "Source-order argument 0; handler/helper role is preserved but finer pixel-formula semantics remain open." },
    NativeParameterSpec { name: "arg1", kind: "i32 / bitmap operand", description: "Source-order argument 1; handler/helper role is preserved but finer pixel-formula semantics remain open." },
    NativeParameterSpec { name: "arg2", kind: "i32 / bitmap operand", description: "Source-order argument 2; handler/helper role is preserved but finer pixel-formula semantics remain open." },
    NativeParameterSpec { name: "arg3", kind: "i32 / bitmap operand", description: "Source-order argument 3; handler/helper role is preserved but finer pixel-formula semantics remain open." },
    NativeParameterSpec { name: "arg4", kind: "i32 / bitmap operand", description: "Source-order argument 4; handler/helper role is preserved but finer pixel-formula semantics remain open." },
    NativeParameterSpec { name: "arg5", kind: "i32 / bitmap operand", description: "Source-order argument 5; handler/helper role is preserved but finer pixel-formula semantics remain open." },
];
const GRAPH90_SYNTHESIZE_BITMAP_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec { name: "arg4", kind: "i32 / bitmap operand", description: "Source-order argument 4; handler/helper role is preserved but finer pixel-formula semantics remain open." },
    NativeParameterSpec { name: "arg5", kind: "i32 / bitmap operand", description: "Source-order argument 5; handler/helper role is preserved but finer pixel-formula semantics remain open." },
    NativeParameterSpec { name: "arg6", kind: "i32 / bitmap operand", description: "Source-order argument 6; handler/helper role is preserved but finer pixel-formula semantics remain open." },
];
...
const GRAPH90_SCALE_BITMAP_REGION_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec { name: "arg5", kind: "i32 / bitmap operand", description: "Source-order argument 5; handler/helper role is preserved but finer pixel-formula semantics remain open." },
    NativeParameterSpec { name: "arg6", kind: "i32 / bitmap operand", description: "Source-order argument 6; handler/helper role is preserved but finer pixel-formula semantics remain open." },
    NativeParameterSpec { name: "arg7", kind: "i32 / bitmap operand", description: "Source-order argument 7; handler/helper role is preserved but finer pixel-formula semantics remain open." },
    NativeParameterSpec { name: "arg8", kind: "i32 / bitmap operand", description: "Source-order argument 8; handler/helper role is preserved but finer pixel-formula semantics remain open." },
    NativeParameterSpec { name: "arg9", kind: "i32 / bitmap operand", description: "Source-order argument 9; handler/helper role is preserved but finer pixel-formula semantics remain open." },
];
...
const GRAPH90_START_NODE_CONTROL_EX_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec { name: "arg1", kind: "control argument", description: "Source-order control argument 1; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
    NativeParameterSpec { name: "arg2", kind: "control argument", description: "Source-order control argument 2; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
    NativeParameterSpec { name: "arg3", kind: "control argument", description: "Source-order control argument 3; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
    NativeParameterSpec { name: "arg4", kind: "control argument", description: "Source-order control argument 4; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
    NativeParameterSpec { name: "arg5", kind: "control argument", description: "Source-order control argument 5; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
    NativeParameterSpec { name: "arg6", kind: "control argument", description: "Source-order control argument 6; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
    NativeParameterSpec { name: "arg7", kind: "control argument", description: "Source-order control argument 7; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
    NativeParameterSpec { name: "arg8", kind: "control argument", description: "Source-order control argument 8; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
    NativeParameterSpec { name: "arg9", kind: "control argument", description: "Source-order control argument 9; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
];
const GRAPH90_START_SPECIAL_OBJECT_CONTROL_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec { name: "arg8", kind: "control argument", description: "Source-order control argument 8; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
    NativeParameterSpec { name: "arg9", kind: "control argument", description: "Source-order control argument 9; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
    NativeParameterSpec { name: "arg10", kind: "control argument", description: "Source-order control argument 10; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
    NativeParameterSpec { name: "arg11", kind: "control argument", description: "Source-order control argument 11; exact field name remains unrecovered while constructor placement and procedure class are confirmed." },
];
...
const GRAPH90_START_SPLINE_OBJECT_CONTROL_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "object", kind: "display object handle", description: "CProcCtrlDspObjBC target CDspObj." },
    NativeParameterSpec { name: "point_count", kind: "i32", description: "Number of 16-byte spline point records; sub_432490 rejects non-positive counts." },
    NativeParameterSpec { name: "points", kind: "pointer", description: "Pointer to point records; X/Y/Z feed three natural CSpline axes and the fourth DWORD is retained as endpoint metadata." },
    NativeParameterSpec { name: "position_curve", kind: "curve id", description: "Position progress curve evaluated by sub_41A690 before CSpline sampling." },
    NativeParameterSpec { name: "target_alpha", kind: "0..256 transparency", description: "Terminal CDspObj transparency." },
    NativeParameterSpec { name: "packed_alpha_curve_scale", kind: "u32", description: "Low 16 bits are alpha curve; high 16 bits are alpha time scale, with zero meaning 0x10000." },
    NativeParameterSpec { name: "fixed_parameter_target", kind: "i32", description: "Terminal selector -1 fixed parameter; negative preserves the captured start value." },
    NativeParameterSpec { name: "duration_ms", kind: "milliseconds", description: "Base CProcCtrlDspObj duration; zero is normalized to one millisecond." },
    NativeParameterSpec { name: "update_denominator", kind: "i32", description: "Denominator used by sub_431D90 for the native catch-up interval." },
    NativeParameterSpec { name: "update_numerator", kind: "i32", description: "Numerator/enable value used by sub_431D90 for catch-up limiting." },
    NativeParameterSpec { name: "input_enabled", kind: "boolean", description: "Enables CProcCtrlDspObj input-interrupt handling." },
    NativeParameterSpec { name: "input_descriptor", kind: "input descriptor", description: "Packed target input scope used by CProcCtrlDspObj::Tick." },
];
...
const GRAPH90_SET_OBJECT_HIT_MASK_BITMAP_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "object", kind: "display object handle", description: "Target-recovered BP argument." },
    NativeParameterSpec { name: "bitmap", kind: "bitmap handle or -1/-2", description: "-1 clears; -2 generates from the object bitmap; nonnegative selects a bitmap descriptor." },
];
...
const GRAPH_SET_OBJECT_ALPHA_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        description:
            "Target setter stores this value at CDspObj+0xAC; 0 is opaque and 256 is transparent.",
    },
];
...
const GRAPH90_CONFIGURE_SPRITE_SCALED_BITMAP_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "sprite", kind: "sprite handle", description: "Target CDspObjSprite." },
    NativeParameterSpec { name: "x", kind: "i32 pixel coordinate", description: "Ordinary CDspObj X anchor written through vtable+0x2C before mode-2 raster-origin correction." },
    NativeParameterSpec { name: "y", kind: "i32 pixel coordinate", description: "Ordinary CDspObj Y anchor written through vtable+0x2C before mode-2 raster-origin correction." },
    NativeParameterSpec { name: "bitmap", kind: "bitmap handle", description: "Primary bitmap; target rejects an invalid bitmap descriptor." },
    NativeParameterSpec { name: "transform_x", kind: "i32 pixel value", description: "Mode-2 transform X term; sub_4275B0 converts it to signed 16.16 and stores it at CDspObjSprite+0x248. It is not a source-rectangle X." },
    NativeParameterSpec { name: "transform_y", kind: "i32 pixel value", description: "Mode-2 transform Y term; sub_4275B0 converts it to signed 16.16 and stores it at CDspObjSprite+0x24C. It is not a source-rectangle Y." },
    NativeParameterSpec { name: "rotation_16_16", kind: "signed 16.16 degrees", description: "Mode-2 rotation term stored at CDspObjSprite+0x250 and consumed by the transformed-bounds helper." },
    NativeParameterSpec { name: "scale_x_16_16", kind: "signed 16.16 ratio", description: "Horizontal stretch ratio. The public wrapper reports an invalid-stretch error for rejected X/Y ratios." },
    NativeParameterSpec { name: "scale_y_16_16", kind: "signed 16.16 ratio", description: "Vertical stretch ratio." },
    NativeParameterSpec { name: "raster_mode_flag", kind: "i32 flag", description: "Mode-2 raster/blit selector stored at CDspObjSprite+0x280 and forwarded to the affine pixel path; exact higher-level label remains unclosed." },
    NativeParameterSpec { name: "blend_mode", kind: "i32 blend selector", description: "Target blend mode." },
    NativeParameterSpec { name: "alpha_parameter", kind: "i32 transparency parameter", description: "Target transparency parameter; 0 is opaque and 256 fully transparent." },
    NativeParameterSpec { name: "priority", kind: "u16 render priority", description: "Display-chain priority." },
];
...
const GRAPH90_REPLACE_MAP_TILE_ID_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        description:
            "Tile identifier replaced in the secondary grid using the target complement rule.",
    },
];
...
const GRAPH90_START_WINDOW_MESSAGE_PROCEDURE_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "window", kind: "window handle", description: "Message destination; target requires a configured font." },
    NativeParameterSpec { name: "text", kind: "Shift-JIS message string", description: "Parsed by CProcDspMsg." },
    NativeParameterSpec { name: "procedure_value", kind: "i32", description: "Forwarded through the target procedure configuration virtual path; concrete field meaning remains open." },
    NativeParameterSpec { name: "completion_control", kind: "i32", description: "Stored at CProcDspMsg+0x30." },
    NativeParameterSpec { name: "end_wait_policy", kind: "i32", description: "Stored at CProcDspMsg+0x38." },
    NativeParameterSpec { name: "allow_high_bit_input", kind: "bool/i32", description: "Stored at CProcDspMsg+0x3C; controls bit-31 input acceptance." },
];
...
const GRAPH90_CARET_POSITION_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec {
        ...
        description:
            "Mode 1 is absolute; other values are relative to the current window message position.",
    },
    NativeParameterSpec {
        name: "x",
...
const GRAPH90_GLYPH_STRIP_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        kind: "i32 0..=255",
        description:
            "Positive values register slices at 0xFF01 onward; nonpositive clears the fixed atlas.",
    },
];
...
const GRAPH90_DRAW_ICON_BATCH_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        kind: "BP pointer",
        description:
            "Base records are 16 bytes; extended records are projected from 64-byte strides.",
    },
];
...
const GRAPH91_MESSAGE_EX_OPTION_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "argument_1", kind: "i32/string native field", description: "Target CProcDspMsgEx constructor argument in BP source order; argument 11 is the extended option." },
    NativeParameterSpec { name: "argument_2", kind: "i32/string native field", description: "Target CProcDspMsgEx constructor argument in BP source order; argument 11 is the extended option." },
    NativeParameterSpec { name: "argument_3", kind: "i32/string native field", description: "Target CProcDspMsgEx constructor argument in BP source order; argument 11 is the extended option." },
    NativeParameterSpec { name: "argument_4", kind: "i32/string native field", description: "Target CProcDspMsgEx constructor argument in BP source order; argument 11 is the extended option." },
    NativeParameterSpec { name: "argument_5", kind: "i32/string native field", description: "Target CProcDspMsgEx constructor argument in BP source order; argument 11 is the extended option." },
    NativeParameterSpec { name: "argument_6", kind: "i32/string native field", description: "Target CProcDspMsgEx constructor argument in BP source order; argument 11 is the extended option." },
    NativeParameterSpec { name: "argument_7", kind: "i32/string native field", description: "Target CProcDspMsgEx constructor argument in BP source order; argument 11 is the extended option." },
    NativeParameterSpec { name: "argument_8", kind: "i32/string native field", description: "Target CProcDspMsgEx constructor argument in BP source order; argument 11 is the extended option." },
    NativeParameterSpec { name: "argument_9", kind: "i32/string native field", description: "Target CProcDspMsgEx constructor argument in BP source order; argument 11 is the extended option." },
    NativeParameterSpec { name: "argument_10", kind: "i32/string native field", description: "Target CProcDspMsgEx constructor argument in BP source order; argument 11 is the extended option." },
    NativeParameterSpec { name: "argument_11", kind: "i32/string native field", description: "Target CProcDspMsgEx constructor argument in BP source order; argument 11 is the extended option." },
];
...
const GRAPH91_RENDER_TEXT_STYLE_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "argument_1", kind: "i32/string renderer field", description: "Immediate renderer argument; the additional flag selects default versus zeroed style state." },
    NativeParameterSpec { name: "argument_2", kind: "i32/string renderer field", description: "Immediate renderer argument; the additional flag selects default versus zeroed style state." },
    NativeParameterSpec { name: "argument_3", kind: "i32/string renderer field", description: "Immediate renderer argument; the additional flag selects default versus zeroed style state." },
    NativeParameterSpec { name: "argument_4", kind: "i32/string renderer field", description: "Immediate renderer argument; the additional flag selects default versus zeroed style state." },
    NativeParameterSpec { name: "argument_5", kind: "i32/string renderer field", description: "Immediate renderer argument; the additional flag selects default versus zeroed style state." },
    NativeParameterSpec { name: "argument_6", kind: "i32/string renderer field", description: "Immediate renderer argument; the additional flag selects default versus zeroed style state." },
];
...
const GRAPH91_TEXT_SUBSTITUTION_RECORDS_PARAMETERS: &[NativeParameterSpec] =
    &[NativeParameterSpec {
        ...
        kind: "string",
        description:
            "Newline-delimited base\\reading records registered atomically until malformed input.",
    }];
...
const GRAPH91_DRAW_TEXT_STYLE_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec {
        ...
        kind: "i32/string/pointer renderer field",
        description:
            "Extended renderer argument; the additional flag selects default versus zeroed style.",
    },
    NativeParameterSpec {
        name: "argument_2",
        kind: "i32/string/pointer renderer field",
...
const GRAPH_BIND_BITMAP_TO_SURFACE_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        description:
            "Second BP argument; not passed to the target core and only used in error diagnostics.",
    },
    NativeParameterSpec {
        name: "frame_error_context",
        kind: "i32 diagnostic context",
        description:
            "Third BP argument; not passed to the target core and only used in error diagnostics.",
    },
    NativeParameterSpec {
        name: "source_bitmap",
        kind: "bitmap handle",
        description:
            "Fourth BP argument; target sub_440780 maps this bitmap to destination_surface.",
    },
];
...
const GRAPH92_PRELOAD_BITMAP_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "resource_name", kind: "script string", description: "Bitmap resource name passed to DCProcPreloadBmp." },
    NativeParameterSpec { name: "context", kind: "script string", description: "Converted diagnostic/context string not passed to the target preload core in this build." },
];
...
const MESSAGE_AUXILIARY_INPUT_MASK_PARAMETERS: &[NativeParameterSpec] = &[NativeParameterSpec {
    ...
    kind: "i32 bit mask",
    description:
        "Stored in target dword_507690 and ORed into the message/procedure input event mask.",
}];
...
const SYS_RESTART_WITH_COMMAND_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "working_directory", kind: "nullable Shift-JIS pointer", description: "Restart working directory; null selects the target default." },
    NativeParameterSpec { name: "command_line", kind: "non-null Shift-JIS pointer", description: "Command executed after shutdown; a null pointer is fatal, while an empty string is valid." },
    NativeParameterSpec { name: "error_message", kind: "nullable Shift-JIS pointer", description: "Optional launch failure message." },
];
...
const SYS_SHOW_INSTALLER_DIALOG_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        description:
            "Second text field; selects the target dialog resource variant (sub_472050 a1).",
    },
    NativeParameterSpec {
        name: "text3",
        kind: "Shift-JIS string",
...
const SYS_RUN_INSTALLER_WORKFLOW_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        kind: "Shift-JIS string",
        description:
            "Optional protected-file format string used by the target uninstall-list writer.",
    },
    NativeParameterSpec {
        name: "vendor",
        kind: "Shift-JIS string",
...
const STRUCTURED_HISTORY_READ_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        kind: "u32 index",
        description:
            "Zero selects the newest record and follows native prev links for older entries.",
    },
];
...
const REGISTERED_OBJECT_MESSAGE_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec {
        ...
        kind: "i32 registry identifier",
        description:
            "Registered object whose variable-length message queue receives the descriptor.",
    },
    NativeParameterSpec {
        name: "count",
...
const EXCLUSION_PRIORITY_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        kind: "u32",
        description:
            "Higher priorities are inserted before lower priorities; equal priorities remain FIFO.",
    },
];
...
const MAIN_LOOP_WAIT_PARAMETERS: &[NativeParameterSpec] = &[NativeParameterSpec {
    ...
    kind: "i32",
    description:
        "Stored unchanged in dword_503EF8 and consulted by the target main-loop pacing branch.",
}];
const EXCLUSIVE_THREAD_PARAMETERS: &[NativeParameterSpec] = &[NativeParameterSpec { name: "enabled", kind: "bool/i32", description: "Nonzero stores the current CThread pointer and enables exclusive scheduling; zero clears both globals." }];
...
const BOOTSTRAP_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        kind: "Shift-JIS string",
        description:
            "Logical archive namespace associated with system.arc or the selected archive basename.",
    },
];
...
const SOUND_SE_LOAD_SCALED_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "channel", kind: "i32 SE channel", description: "Resident SE slot 0..63." },
    NativeParameterSpec { name: "archive", kind: "Shift-JIS archive/root", description: "Archive or resource root." },
    NativeParameterSpec { name: "file", kind: "Shift-JIS resource name", description: "Sound-effect resource." },
    NativeParameterSpec { name: "native_start_parameter", kind: "i32", description: "Opaque value forwarded to the target sound-object start/configuration virtual call; it is not a fade duration." },
    NativeParameterSpec { name: "decode_gain_16_16", kind: "signed 16.16 fixed point", description: "Decoder/sample gain scale." },
];
...
const SOUND_SE_LOAD_CUSTOM_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        kind: "i32",
        description:
            "Opaque value forwarded to the target sound-object start/configuration virtual call.",
    },
    NativeParameterSpec {
...
const USER_DRAW_BITMAP_TO_WINDOW_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        description:
            "Bitmap resource below 0x4000 resolved through the target graph resource manager.",
    },
];
...
const USER_SHOW_TYPED_MESSAGE_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "message", kind: "Shift-JIS string", description: "Message text." },
    NativeParameterSpec { name: "kind", kind: "i32", description: "One selects OK/cancel-style success on IDOK; other values select yes/no success on IDYES." },
    NativeParameterSpec { name: "topmost", kind: "BOOL", description: "Controls the target task-modal flag." },
];
...
const USER_SHOW_INPUT_DIALOG_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        kind: "signed i32",
        description:
            "Absolute value is the byte limit; a negative value enables signed-decimal filtering.",
    },
];
...
const USER_SHOW_SELECTION_DIALOG_PARAMETERS: &[NativeParameterSpec] = &[
    NativeParameterSpec { name: "buffer", kind: "BP pointer to mutable Shift-JIS buffer", description: "Receives four edited segments joined by '-'." },
    NativeParameterSpec { name: "title", kind: "Shift-JIS string", description: "Dialog window title." },
    NativeParameterSpec { name: "prompt", kind: "Shift-JIS string", description: "Prompt shown above the four segment fields." },
    NativeParameterSpec { name: "segment_max_length", kind: "signed i32", description: "Absolute per-segment byte limit, capped to 32; negative enables signed-decimal filtering." },
    NativeParameterSpec { name: "extra", kind: "i32", description: "Target positional option stored by the native dialog." },
];
...
const USER_C0_09_PARAMETERS: &[NativeParameterSpec] = &[
    ...
    NativeParameterSpec {
        ...
        description:
            "Zero removes the object from the automatic update list; nonzero installs/updates it.",
    },
];
...
const DOCUMENTED_OPCODES: &[NativeOpcodeSpec] = &[
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SET_CLOCK_JUMP_THRESHOLD, symbol: "Sys81_04_SetClockJumpThreshold", parameters: SYS81_04_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_498820 validates 50..60000 and stores the engine-clock discontinuity threshold.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_COPY_COORDINATE_SLOT, symbol: "Sys81_07_CopyCoordinateSlot", parameters: SYS81_07_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_48E590 copies one of five two-DWORD coordinate transform slots.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_GET_USER_NAME, symbol: "Sys81_08_GetUserName", parameters: SYS81_08_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target calls GetUserNameA with a 257-byte capacity.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_GET_COMPUTER_NAME, symbol: "Sys81_09_GetComputerName", parameters: SYS81_09_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target calls GetComputerNameA with a 16-byte capacity.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_GET_CPU_BRAND, symbol: "Sys81_0A_GetCpuBrand", parameters: SYS81_0A_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_46F720 gates extended CPUID leaves 0x80000002..4 and normalizes whitespace.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_GET_CPU_DISPLAY_INFO, symbol: "Sys81_0B_GetCpuDisplayInfo", parameters: SYS81_0B_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_45E490 normalizes CPU text and copies four cached 16-bit signature fields as DWORDs.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_GET_OS_VERSION, symbol: "Sys81_0C_GetOsVersion", parameters: SYS81_0C_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target copies cached GetVersionExA data.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_GET_PHYSICAL_MEMORY_MB, symbol: "Sys81_0D_GetPhysicalMemoryMb", parameters: SYS81_0D_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target calls GlobalMemoryStatusEx and shifts both physical-memory values right by 20.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_GET_ADJUSTED_DESKTOP_DIMENSIONS, symbol: "Sys81_0E_GetAdjustedDesktopDimensions", parameters: SYS81_0E_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_45E580 applies the wide/tall desktop halving rules.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_IS_MAIN_WINDOW_MINIMIZED, symbol: "Sys81_0F_IsMainWindowMinimized", parameters: NO_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target directly returns IsIconic(hWndParent).",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SWAP_INPUT_BINDING, symbol: "Sys81_10_SwapInputBinding", parameters: SYS81_10_PARAMETERS,
        returns: "previous value", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_46DA40 returns the old dword_518CAC[6*index] then stores the replacement.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_COPY_KEYBOARD_STATE, symbol: "Sys81_11_CopyKeyboardState", parameters: SYS81_11_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target directly calls GetKeyboardState.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SET_KEYBOARD_POLLING_OVERRIDE, symbol: "Sys81_14_SetKeyboardPollingOverride", parameters: SYS81_14_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_46D540 controls the raw keyboard polling override consumed by sub_46D560.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_CONFIGURE_POINTER_HISTORY, symbol: "Sys81_16_ConfigurePointerHistory", parameters: SYS81_16_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target clears the linked history and accepts capacities up to 512.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_COPY_POINTER_HISTORY, symbol: "Sys81_17_CopyPointerHistory", parameters: SYS81_17_PARAMETERS,
        returns: "sample count", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target traverses the pointer-history linked list newest-first.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_REGISTER_TOUCH_INPUT, symbol: "Sys81_18_RegisterTouchInput", parameters: SYS81_18_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target calls RegisterTouchWindow or UnregisterTouchWindow for hWndParent.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_COPY_TOUCH_RECORDS, symbol: "Sys81_19_CopyTouchRecords", parameters: SYS81_19_PARAMETERS,
        returns: "record count", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target copies each touch record as six DWORDs.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SET_CONTROLLER_WAKE_ENTRY, symbol: "Sys81_1B_SetControllerWakeEntry", parameters: SYS81_1B_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_460E40 writes one of 36 controller wake values.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_QUERY_CONTROLLER_STATE, symbol: "Sys81_1D_QueryControllerState", parameters: SYS81_1D_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_460E60 aggregates DirectInput device state and clamps three axes to -1024..1024.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_INJECT_MOUSE_CLICK, symbol: "Sys81_1E_InjectMouseClick", parameters: SYS81_1E_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target posts the matching synthetic mouse down/up messages.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_OPEN_RESOURCE_STREAM, symbol: "Sys81_28_OpenResourceStream", parameters: SYS81_28_PARAMETERS,
        returns: "status 0..3", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_4011B0 returns 1 invalid mode, 2 duplicate path, 3 open failure, or 0.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_QUEUE_RESOURCE_CLOSE, symbol: "Sys81_29_QueueResourceClose", parameters: SYS81_29_PARAMETERS,
        returns: "zero or four", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target queues a close operation; missing handles map to status 4.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_QUEUE_RESOURCE_READ, symbol: "Sys81_2A_QueueResourceRead", parameters: SYS81_2A_PARAMETERS,
        returns: "zero or four", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_4014A0 queues a read request on the stream worker list.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_QUEUE_RESOURCE_SEEK, symbol: "Sys81_2B_QueueResourceSeek", parameters: SYS81_2B_PARAMETERS,
        returns: "zero or four", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_4014C0 queues a no-buffer stream operation at the supplied offset.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_GET_FILE_TIMES, symbol: "Sys81_2C_GetFileTimes", parameters: SYS81_2C_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target opens the file abstraction, queries three FILETIMEs and converts each to SYSTEMTIME.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SET_FILE_TIMES, symbol: "Sys81_2D_SetFileTimes", parameters: SYS81_2D_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target converts three SYSTEMTIMEs to FILETIME and applies them to the opened file.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_TEST_PATH_WRITABLE, symbol: "Sys81_2F_TestPathWritable", parameters: SYS81_2F_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target creates, reopens and deletes a temporary file in the directory.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_READ_RESOURCE_BINARY, symbol: "Sys81_30_ReadResourceBinary", parameters: SYS81_30_PARAMETERS,
        returns: "sync status or procedure result", scheduling: NativeSchedulingEffect::WaitProcedure,
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_INTERNET_READ, symbol: "Sys81_31_InternetRead", parameters: SYS81_31_PARAMETERS,
        returns: "status -1,0,1,2,3", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target WinINet helper distinguishes initialization, open, read and short-read failures.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_DIRECT_FILE_READ, symbol: "Sys81_32_DirectFileRead", parameters: SYS81_32_PARAMETERS,
        returns: "status 0,1,8,-1", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target uses unbuffered/direct file I/O and validates a drive-letter path.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_RESOURCE_SIZE, symbol: "Sys81_35_ResourceSize", parameters: SYS81_35_PARAMETERS,
        returns: "byte count or zero", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_468310 returns the resource size.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_ENUMERATE_DRIVE_TYPES, symbol: "Sys81_36_EnumerateDriveTypes", parameters: SYS81_36_PARAMETERS,
        returns: "nonzero drive count", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target maps GetDriveTypeA to BGI codes: fixed 1, removable 2, network 3, CD-ROM 4, RAM disk 5.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_GET_DISK_FREE_MB, symbol: "Sys81_37_GetDiskFreeMb", parameters: SYS81_37_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target queries free bytes and shifts right by 20.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SHOW_RESOURCE_FILE_DIALOG, symbol: "Sys81_38_ShowResourceFileDialog", parameters: SYS81_38_PARAMETERS,
        returns: "status", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target builds parallel filter-label and extension arrays then calls the Win32 dialog helper.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_ENUMERATE_RESOURCES, symbol: "Sys81_39_EnumerateResources", parameters: SYS81_39_PARAMETERS,
        returns: "entry count/status", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_467A60 enumerates matching archive/resource names into packed NUL strings.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_BROWSE_FOLDER, symbol: "Sys81_3A_BrowseFolder", parameters: SYS81_3A_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target opens SHBrowseForFolder and converts the selected PIDL to a path.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SHOW_RESOURCE_LIST, symbol: "Sys81_3B_ShowResourceList", parameters: SYS81_3B_PARAMETERS,
        returns: "selection/status", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target enumerates resources, joins names with newlines and opens a modal list.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_RESOURCE_EXISTS, symbol: "Sys81_3C_ResourceExists", parameters: SYS81_3C_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_4685D0 tests the resource abstraction rather than only the host filesystem.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_GET_VOLUME_LABEL, symbol: "Sys81_3D_GetVolumeLabel", parameters: SYS81_3D_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_468670 calls GetVolumeInformationA.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_QUERY_DEVICE_POWER_STATE, symbol: "Sys81_3E_QueryDevicePowerState", parameters: SYS81_3E_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target returns whether the query mechanism opened/succeeded and writes the device power state.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_CREATE_CHILD_THREAD, symbol: "Sys81_44_CreateChildThread", parameters: SYS81_44_PARAMETERS,
        returns: "integer thread ID", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target constructs DCTChildThread, validates sizes/index and returns CThread+8.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_CONFIGURE_SCREEN_MODE, symbol: "Sys81_60_ConfigureScreenMode", parameters: SYS81_60_PARAMETERS,
        returns: "status 0,1,2", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_461040 validates slot and nonzero dimensions.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_QUERY_EFFECTIVE_DISPLAY_MODE, symbol: "Sys81_61_QueryEffectiveDisplayMode", parameters: NO_PARAMETERS,
        returns: "mode 0..2", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target returns the configured mode, but mode 2 falls back to zero when adjusted desktop dimensions are too small.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SET_MONITOR_ADAPTER_MODE, symbol: "Sys81_62_SetMonitorAdapterMode", parameters: SYS81_62_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_45E830 stores the monitor/adapter selection mode.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SET_CONFIG_INPUT_MODE, symbol: "Sys81_63_SetConfigInputMode", parameters: SYS81_63_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_45E860 stores the display/input configuration mode.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_CONFIGURE_LOGICAL_SCREEN_SIZE, symbol: "Sys81_64_ConfigureLogicalScreenSize", parameters: SYS81_64_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_45E770 updates logical dimensions and triggers render reconfiguration.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SET_WINDOW_POSITION_OVERRIDE, symbol: "Sys81_65_SetWindowPositionOverride", parameters: SYS81_65_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target value is consumed by WM_WINDOWPOSCHANGING position override logic.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SWAP_PAUSE_ON_DEACTIVATE, symbol: "Sys81_68_SwapPauseOnDeactivate", parameters: SYS81_68_PARAMETERS,
        returns: "previous value", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_498800 swaps dword_566A4C and returns the old value.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SET_PRINT_SCREEN_HOTKEY_CAPTURE, symbol: "Sys81_69_SetPrintScreenHotkeyCapture", parameters: SYS81_69_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target registers/unregisters sixteen PrintScreen hotkeys for modifiers 0..15.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SET_ERROR_CAPTURE_MODE, symbol: "Sys81_6A_SetErrorCaptureMode", parameters: SYS81_6A_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_464440 selects whether engine error text is captured.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_COPY_CAPTURED_ERROR, symbol: "Sys81_6B_CopyCapturedError", parameters: SYS81_6B_PARAMETERS,
        returns: "byte length including NUL", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_464480 copies dword_566004 when non-null and returns strlen+1.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_QUERY_PIXEL_SHADER_VERSION, symbol: "Sys81_6D_QueryPixelShaderVersion", parameters: NO_PARAMETERS,
        returns: "low 16-bit D3D shader version", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_460590 reads the D3DCAPS9 field at offset 0xCC.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_UNCAUGHT_EXCEPTION_ACTIVE, symbol: "Sys81_6E_UncaughtExceptionActive", parameters: NO_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target directly returns the Microsoft CRT __uncaught_exception result.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SET_BICUBIC_SHADER, symbol: "Sys81_6F_SetBicubicShader", parameters: SYS81_6F_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_460550 manages the D3DX bicubic scaler shader.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_WIDE_STRING_DISTANCE, symbol: "Sys81_B0_WideStringDistance", parameters: SYS81_B0_PARAMETERS,
        returns: "insertion/deletion distance or -1", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_495C50 computes len(left)+len(right)-LCS(left,right).",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_SHIFT_JIS_TO_UTF16, symbol: "Sys81_B7_ShiftJisToUtf16", parameters: SYS81_B7_PARAMETERS,
        returns: "UTF-16 unit count or -1", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target converts single-byte and Shift-JIS double-byte code units without a Windows normalization pass.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_BLOB_TABLE_OPEN, symbol: "Sys81_D0_BlobTableOpen", parameters: SYS81_D0_PARAMETERS,
        returns: "zero or 0x80000001", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target creates a growable array of {pointer,size} blob slots.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_BLOB_TABLE_CLOSE, symbol: "Sys81_D1_BlobTableClose", parameters: SYS81_D1_PARAMETERS,
        returns: "zero or 0x80000002", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target removes the matching table node and frees all blobs.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_BLOB_TABLE_STORE, symbol: "Sys81_D2_BlobTableStore", parameters: SYS81_D2_PARAMETERS,
        returns: "status", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target stores into a requested slot or the first free slot; allocation failure maps to 0x80000003.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_BLOB_TABLE_REMOVE, symbol: "Sys81_D3_BlobTableRemove", parameters: SYS81_D3_PARAMETERS,
        returns: "status", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target clears and frees one occupied slot; invalid slot maps to 0x80000004.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_BLOB_TABLE_FETCH, symbol: "Sys81_D4_BlobTableFetch", parameters: SYS81_D4_PARAMETERS,
        returns: "status", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target copies one slot and reports 0x80000004 when absent.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_BLOB_TABLE_ENUMERATE, symbol: "Sys81_D5_BlobTableEnumerate", parameters: SYS81_D5_PARAMETERS,
        returns: "status", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target emits index/size pairs for occupied slots.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_LAUNCH_PROCESS_WAIT, symbol: "Sys81_E0_LaunchProcessWait", parameters: SYS81_E0_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_4724B0 launches, waits while pumping BGI messages, and optionally reads exit code.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_UPDATE_ROLLING_HASH, symbol: "Sys81_E9_UpdateRollingHash", parameters: SYS81_E9_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_401900 updates a 233-multiplier DWORD plus four byte accumulators.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_MD5, symbol: "Sys81_EA_Md5", parameters: SYS81_EA_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target sub_4A1440 pads with the MD5 constants/rounds and writes four digest DWORDs.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_CREATE_NAMED_MUTEX, symbol: "Sys81_EC_CreateNamedMutex", parameters: SYS81_EC_PARAMETERS,
        returns: "integer handle or zero", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target rejects pre-existing mutex names and returns a monotonic BGI handle.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_RELEASE_NAMED_MUTEX, symbol: "Sys81_ED_ReleaseNamedMutex", parameters: SYS81_ED_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target releases/closes the native mutex and removes its BGI record.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_RUN_INSTALLATION_PROCEDURE, symbol: "Sys81_F2_RunInstallationProcedure", parameters: SYS81_F2_PARAMETERS,
        returns: "no immediate value on success; immediate -1/1/2/3 on initialization failure", scheduling: NativeSchedulingEffect::WaitProcedure,
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS81_VALIDATE_OR_CREATE_USER_PATH, symbol: "Sys81_F7_CreateSpecialFolderShortcut", parameters: SYS81_F7_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS_SRAND, symbol: "Sys80_00_Srand", parameters: RNG_SEED_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target handler 0x00487D20 pops one DWORD and calls the imported Microsoft CRT srand directly.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS_RAND, symbol: "Sys80_01_Rand", parameters: NO_PARAMETERS,
        returns: "Microsoft CRT rand value", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target handler 0x00487D40 calls the imported rand and pushes its result unchanged.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS_RAND_MAX, symbol: "Sys80_02_RandMax", parameters: RAND_MAX_PARAMETERS,
        returns: "value in 0..max_exclusive-1, or zero for max<=0", scheduling: NativeSchedulingEffect::Continue,
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS_GET_ENGINE_TICK, symbol: "Sys80_04_GetEngineTick", parameters: NO_PARAMETERS,
        returns: "engine milliseconds", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target 0x00487DB0 calls sub_498720, the pause-adjusted GetTickCount/timeGetTime engine clock.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS_QUERY_PERFORMANCE_COUNTER_NS, symbol: "Sys80_05_QueryPerformanceCounterNs", parameters: PERFORMANCE_COUNTER_PARAMETERS,
        returns: "boolean success", scheduling: NativeSchedulingEffect::Continue,
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS_SET_PERFORMANCE_PROFILING, symbol: "Sys80_06_SetPerformanceProfiling", parameters: PERFORMANCE_PROFILING_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS_READ_PERFORMANCE_METRIC, symbol: "Sys80_07_ReadPerformanceMetric", parameters: PERFORMANCE_METRIC_PARAMETERS,
        returns: "void; metric is written through output_ptr", scheduling: NativeSchedulingEffect::Continue,
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS_READ_CURSOR_POINT, symbol: "Sys80_08_ReadCursorPoint", parameters: NO_PARAMETERS,
        returns: "two values: client-space x, then y", scheduling: NativeSchedulingEffect::Continue,
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS_QUERY_PRESENTATION_STATE, symbol: "Sys80_09_QueryPresentationState", parameters: NO_PARAMETERS,
        returns: "renderer presentation state/timestamp", scheduling: NativeSchedulingEffect::Continue,
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS_COPY_GRAPHICS_CAPABILITIES, symbol: "Sys80_0A_CopyGraphicsCapabilities", parameters: GRAPHICS_CAPABILITY_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target 0x00487EB0 copies exactly 0x40 bytes from dword_518920, the graphics initialization/capability cache.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS_QUERY_GRAPHICS_MEMORY_METRIC, symbol: "Sys80_0B_QueryGraphicsMemoryMetric", parameters: NO_PARAMETERS,
        returns: "renderer-owned capacity/memory field", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target 0x00487EE0 reaches sub_431130 and returns the DWORD at the active renderer object offset +0x58.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS_GET_LOCAL_TIME, symbol: "Sys80_0C_GetLocalTime", parameters: SYSTEM_TIME_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target 0x00487F00 passes the BP destination directly to Win32 GetLocalTime.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS_GET_PHYSICAL_MEMORY, symbol: "Sys80_0D_GetPhysicalMemory", parameters: NO_PARAMETERS,
        returns: "two values: total physical bytes, then available physical bytes", scheduling: NativeSchedulingEffect::Continue,
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS_QUERY_WINDOW_MINIMIZE_LATCH, symbol: "Sys80_0E_QueryWindowMinimizeLatch", parameters: NO_PARAMETERS,
        returns: "boolean minimize/restore latch", scheduling: NativeSchedulingEffect::Continue,
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS_QUERY_WINDOW_ACTIVE, symbol: "Sys80_0F_QueryWindowActive", parameters: NO_PARAMETERS,
        returns: "boolean active-window state", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target 0x00487FD0 returns dword_5666E8, written from WM_ACTIVATE and WM_NCACTIVATE processing.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS_RESET_INPUT_CONFIGURATION, symbol: "Sys80_10_ResetInputConfiguration", parameters: INPUT_CONFIGURATION_PARAMETERS,
        returns: "void", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target 0x00487FF0 stores dword_506A44 through sub_46D970, then sub_46DA20 clears every six-DWORD input-state record.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS_QUERY_KEY_DOWN, symbol: "Sys80_11_QueryKeyDown", parameters: KEY_STATE_PARAMETERS,
        returns: "zero or one", scheduling: NativeSchedulingEffect::Continue,
    ...
    NativeOpcodeSpec {
        opcode: opcodes::SYS_SUM_INPUT_DESCRIPTOR_STATE, symbol: "Sys80_12_SumInputDescriptorState", parameters: INPUT_DESCRIPTOR_LIST_PARAMETERS,
        returns: "sum of current descriptor count/state fields", scheduling: NativeSchedulingEffect::Continue,
        notes: "Target 0x00488040 traverses a descriptor chain and sums dword_518CA4[6*descriptor] through sub_46DC70.",
    },
    NativeOpcodeSpec {
        opcode: opcodes::SYS_SET_MOUSE_BUTTON_MAPPING_MODE, symbol: "Sys80_1E_SetMouseButtonMappingMode", parameters: MOUSE_BUTTON_MAPPING_PARAMETERS,
        returns: "boolean accepted", scheduling: NativeSchedulingEffect::Continue,
    ...
    NativeOpcodeSpec { opcode: opcodes::SYS_ALLOC, symbol: "Sys80_20_Alloc", parameters: ALLOC_PARAMETERS, returns: "opaque BP pointer", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488380 validates the requested size, allocates through sub_48DC70's memory-class table and raises the native VM error path when allocation returns zero." },
    NativeOpcodeSpec { opcode: opcodes::SYS_FREE, symbol: "Sys80_21_Free", parameters: FREE_PARAMETERS, returns: "boolean success", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x004883D0 treats null as success; non-null handles are decoded by sub_48DD10 and invalid/double-free values enter the native VM error path." },
    NativeOpcodeSpec { opcode: opcodes::SYS_COUNT_FILES, symbol: "Sys80_24_CountFiles", parameters: COUNT_FILES_PARAMETERS, returns: "matched file count", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488450 calls sub_466BB0 with no output buffer, unlimited count and the caller's recursive flag." },
    NativeOpcodeSpec { opcode: opcodes::SYS_ENUMERATE_FILES, symbol: "Sys80_25_EnumerateFiles", parameters: ENUMERATE_FILES_PARAMETERS, returns: "entry count, required byte count when destination is null, or -1 on overflow", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488490/sub_466BB0 emits packed NUL-terminated names, skips directories in the primary pass and optionally recurses." },
    NativeOpcodeSpec { opcode: opcodes::SYS_ENUMERATE_DIRECTORIES, symbol: "Sys80_26_EnumerateDirectories", parameters: ENUMERATE_DIRECTORIES_PARAMETERS, returns: "directory count, required byte count when destination is null, or -1 on overflow", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488510/sub_466F00 enumerates immediate directories, excluding dot and dot-dot." },
    NativeOpcodeSpec { opcode: opcodes::SYS_MOVE_FILE, symbol: "Sys80_27_MoveFile", parameters: TWO_PATH_PARAMETERS, returns: "Win32 BOOL", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488560 calls MoveFileA(first native pop, second native pop), which corresponds to the target's destination-first BP argument order documented here." },
    NativeOpcodeSpec { opcode: opcodes::SYS_CREATE_DIRECTORY, symbol: "Sys80_28_CreateDirectory", parameters: PATH_PARAMETER, returns: "Win32 BOOL", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488590 calls CreateDirectoryA(path, NULL) once; it does not recursively create parent directories." },
    NativeOpcodeSpec { opcode: opcodes::SYS_REMOVE_DIRECTORY, symbol: "Sys80_29_RemoveDirectory", parameters: PATH_PARAMETER, returns: "Win32 BOOL", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x004885C0 calls RemoveDirectoryA directly." },
    NativeOpcodeSpec { opcode: opcodes::SYS_DIRECTORY_EXISTS, symbol: "Sys80_2A_DirectoryExists", parameters: PATH_PARAMETER, returns: "zero or one", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x004885F0 returns bit 4 of GetFileAttributesA when attributes are valid, otherwise zero." },
    NativeOpcodeSpec { opcode: opcodes::SYS_SPLIT_PATH, symbol: "Sys80_2B_SplitPath", parameters: SPLIT_PATH_PARAMETERS, returns: "zero or one", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488630 converts the source through MultiByteToWideChar, calls _wsplitpath, and converts requested components back to multibyte buffers. The previous ConvertDebugRecord label was incorrect." },
    NativeOpcodeSpec { opcode: opcodes::SYS_GET_FILE_ATTRIBUTES, symbol: "Sys80_2C_GetFileAttributes", parameters: PATH_PARAMETER, returns: "raw u32 Win32 attributes or 0xFFFFFFFF", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x004886A0 forwards GetFileAttributesA unchanged." },
    NativeOpcodeSpec { opcode: opcodes::SYS_SET_FILE_ATTRIBUTES, symbol: "Sys80_2D_SetFileAttributes", parameters: SET_ATTRIBUTES_PARAMETERS, returns: "Win32 BOOL", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x004886D0 passes the path and complete attribute mask to SetFileAttributesA." },
    NativeOpcodeSpec { opcode: opcodes::SYS_COPY_FILE, symbol: "Sys80_2F_CopyFile", parameters: TWO_PATH_PARAMETERS, returns: "Win32 BOOL", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488700 clears FILE_ATTRIBUTE_READONLY on an existing destination, then calls CopyFileA(source, destination, FALSE)." },
    NativeOpcodeSpec { opcode: opcodes::SYS_READ_FILE_BYTES, symbol: "Sys80_30_ReadFileBytes", parameters: READ_FILE_PARAMETERS, returns: "byte count, or zero after target retry/error handling", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488750/sub_465AB0 resolves the file against configured roots, fills the destination record/buffer and returns the loaded size." },
    NativeOpcodeSpec { opcode: opcodes::SYS_READ_FILE_RANGE, symbol: "Sys80_31_ReadFileRange", parameters: READ_FILE_RANGE_PARAMETERS, returns: "status 0,1,2,3,5,or 6", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488790 packs offset/length for sub_465C30. Zero/zero copies the whole file; 2 denotes range overflow, 3 invalid length, 5 read/decode failure, and 6 a source larger than 64 MiB." },
    NativeOpcodeSpec { opcode: opcodes::SYS_WRITE_FILE_BYTES, symbol: "Sys80_32_WriteFileBytes", parameters: WRITE_FILE_PARAMETERS, returns: "zero or one", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x004887F0 returns whether sub_465E30 wrote exactly the requested byte count." },
    NativeOpcodeSpec { opcode: opcodes::SYS_DELETE_FILE, symbol: "Sys80_33_DeleteFile", parameters: DELETE_FILE_PARAMETERS, returns: "Win32 BOOL", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488840 constructs either root\\file or primary_root+file, then calls DeleteFileA." },
    NativeOpcodeSpec { opcode: opcodes::SYS_FILE_EXISTS, symbol: "Sys80_34_FileExists", parameters: FILE_EXISTS_PARAMETERS, returns: "target search-root status", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x004888C0 pops file first and archive/root second, then calls sub_4665C0 with ECX=file and the archive/root stack argument; both values participate in loose-file and archive lookup." },
    NativeOpcodeSpec { opcode: opcodes::SYS_FILE_SIZE, symbol: "Sys80_35_FileSize", parameters: FILE_SIZE_PARAMETERS, returns: "byte count or zero", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488900 pops file first and archive/root second, then calls sub_4662E0 with ECX=archive/root and file on the stack; the helper returns zero on failure." },
    NativeOpcodeSpec { opcode: opcodes::SYS_SET_ADDITIONAL_RESOURCE_SEARCH, symbol: "Sys80_36_SetAdditionalResourceSearch", parameters: RESOURCE_SEARCH_ENABLE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "Target sub_465240 stores the value in dword_506BE0; it is a mode flag, not a commit operation." },
    NativeOpcodeSpec { opcode: opcodes::SYS_PREPEND_RESOURCE_SEARCH_PATH, symbol: "Sys80_37_PrependResourceSearchPath", parameters: RESOURCE_SEARCH_PATH_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "Target allocates a path node and prepends it to the linked list rooted at dword_566630." },
    NativeOpcodeSpec { opcode: opcodes::SYS_REGISTER_COMPOSITE_ARCHIVE, symbol: "Sys80_38_RegisterCompositeArchive", parameters: COMPOSITE_ARCHIVE_PARAMETERS, returns: "zero or one", scheduling: NativeSchedulingEffect::Continue, notes: "Target consumes a zero-terminated pointer array and constructs/registers a named DCArchiveComplex." },
    NativeOpcodeSpec { opcode: opcodes::SYS_SET_VALIDATED_FILE_ROOT, symbol: "Sys80_39_SetValidatedFileRoot", parameters: PATH_PARAMETER, returns: "zero or one", scheduling: NativeSchedulingEffect::Continue, notes: "Target accepts an existing directory and stores it with a trailing backslash in byte_518978." },
    NativeOpcodeSpec { opcode: opcodes::SYS_GET_SPECIAL_FOLDER, symbol: "Sys80_3A_GetSpecialFolder", parameters: SPECIAL_FOLDER_PARAMETERS, returns: "zero or one", scheduling: NativeSchedulingEffect::Continue, notes: "Target selects six Win32 system/shell folder sources, including ProgramFilesDir from the registry, and copies the resulting path." },
    NativeOpcodeSpec { opcode: opcodes::SYS_OPEN_FILE_DIALOG, symbol: "Sys80_3B_OpenFileDialog", parameters: FILE_DIALOG_PARAMETERS, returns: "0 success, -1 cancel, or validation status", scheduling: NativeSchedulingEffect::Continue, notes: "Target builds one extension filter and calls GetOpenFileNameA/GetSaveFileNameA. The portable host now invokes a blocking native/scripted chooser on macOS, Windows and Linux while preserving target status codes." },
    NativeOpcodeSpec { opcode: opcodes::SYS_REQUIRE_RESOURCE_FILE, symbol: "Sys80_3C_RequireResourceFile", parameters: REQUIRE_RESOURCE_PARAMETERS, returns: "zero or one", scheduling: NativeSchedulingEffect::Continue, notes: "Target repeatedly searches configured roots and prompts Retry/Quit. The portable host performs the same loop through native macOS, Windows, or Linux message dialogs." },
    NativeOpcodeSpec { opcode: opcodes::SYS_GET_CONFIGURED_ROOT, symbol: "Sys80_3D_GetConfiguredRoot", parameters: CONFIGURED_ROOT_PARAMETERS, returns: "zero or one", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488B20/sub_4649A0 copies the primary root for kind 0, or the configured secondary root for kind 1; other values fail." },
    NativeOpcodeSpec { opcode: opcodes::SYS_SET_PRIMARY_ROOT, symbol: "Sys80_3E_SetPrimaryRoot", parameters: PATH_PARAMETER, returns: "zero or one", scheduling: NativeSchedulingEffect::Continue, notes: "Target 0x00488B50 accepts only an existing directory, writes path plus a trailing backslash to the primary global root buffer and returns one." },
    NativeOpcodeSpec { opcode: opcodes::SYS_CONFIGURE_REMOVABLE_ARCHIVE, symbol: "Sys80_3F_ConfigureRemovableArchive", parameters: REMOVABLE_ARCHIVE_PARAMETERS, returns: "zero or one", scheduling: NativeSchedulingEffect::Continue, notes: "Target scans removable/CD drives for the configured archive marker and stores the successful drive/subdirectory as the secondary root. The portable host scans platform mount roots and preserves the retry loop." },
    NativeOpcodeSpec { opcode: opcodes::SYS_LOAD_PROGRAM_MODULE, symbol: "Sys80_40_LoadProgramModule", parameters: LOAD_PROGRAM_MODULE_PARAMETERS, returns: "integer code-region base offset", scheduling: NativeSchedulingEffect::Continue, notes: "Target sub_488C00/sub_444CE0 appends decoded BP code to the current CThread and returns the previous code-used end; it does not return an object handle." },
    NativeOpcodeSpec { opcode: opcodes::SYS_FREE_LAST_PROGRAM_MODULE, symbol: "Sys80_41_FreeLastProgramModule", parameters: FREE_PROGRAM_ABI_SLOT_PARAMETERS, returns: "remaining module count", scheduling: NativeSchedulingEffect::Continue, notes: "Target sub_444D80 removes the tail module record and rolls back CThread code-used state. The handler performs no native argument pop despite the legacy ABI descriptor count." },
    NativeOpcodeSpec { opcode: opcodes::SYS_LOAD_PROGRAM_THREAD, symbol: "Sys80_44_LoadProgramThread", parameters: LOAD_PROGRAM_THREAD_PARAMETERS, returns: "integer CThread identifier", scheduling: NativeSchedulingEffect::Continue, notes: "Target creates a child CThread, allocates its regions, appends the initial module and returns CThread+8." },
    NativeOpcodeSpec { opcode: opcodes::SYS_RTC_NOOP, symbol: "Sys80_45_RtcNoop", parameters: NO_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "The release-image selector table points at __RTC_NumErrors, but the BP ABI exposes no output and no useful engine side effect." },
    NativeOpcodeSpec { opcode: opcodes::SYS_CURRENT_THREAD_ID, symbol: "Sys80_46_CurrentThreadId", parameters: NO_PARAMETERS, returns: "integer CThread identifier", scheduling: NativeSchedulingEffect::Continue, notes: "Target sub_488D80 returns the current CThread field at +8." },
    NativeOpcodeSpec { opcode: opcodes::SYS_THREAD_EXISTS, symbol: "Sys80_47_ThreadExists", parameters: THREAD_ID_PARAMETERS, returns: "zero or one", scheduling: NativeSchedulingEffect::Continue, notes: "Target recursively searches the CThread tree; identifier zero is never reported as present." },
    NativeOpcodeSpec {
    ...
    NativeOpcodeSpec { opcode: opcodes::SYS_LOAD_GLOBAL_USER_DATA, symbol: "Sys80_80_LoadGlobalUserData", parameters: NO_PARAMETERS, returns: "window_x, window_y, status", scheduling: NativeSchedulingEffect::Continue, notes: "Target loads BGI.gdb, decodes SDC data and maps missing/decode failures to status 1/2." },
    NativeOpcodeSpec { opcode: opcodes::SYS_SAVE_GLOBAL_USER_DATA, symbol: "Sys80_81_SaveGlobalUserData", parameters: NO_PARAMETERS, returns: "boolean success", scheduling: NativeSchedulingEffect::Continue, notes: "Target writes the BURIKO GDB 3.00 header, global data, resource names and read-flag tables to BGI.gdb." },
    NativeOpcodeSpec { opcode: opcodes::SYS_WRITE_GLOBAL_DATA_BLOCK, symbol: "Sys80_82_WriteGlobalDataBlock", parameters: GLOBAL_DATA_BLOCK_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "Target copies into the fixed 1 MiB global data region and treats zero/out-of-range transfers as fatal." },
    NativeOpcodeSpec { opcode: opcodes::SYS_READ_GLOBAL_DATA_BLOCK, symbol: "Sys80_83_ReadGlobalDataBlock", parameters: GLOBAL_DATA_BLOCK_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "Target copies from the fixed 1 MiB global data region and treats zero/out-of-range transfers as fatal." },
    NativeOpcodeSpec { opcode: opcodes::SYS_RESET_STRUCTURED_HISTORY, symbol: "Sys80_90_ResetStructuredHistory", parameters: STRUCTURED_HISTORY_CAPACITY_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "Frees all native 0x40-byte history nodes, clears head/tail/count and stores the new capacity." },
    NativeOpcodeSpec { opcode: opcodes::SYS_STRUCTURED_HISTORY_COUNT, symbol: "Sys80_91_StructuredHistoryCount", parameters: NO_PARAMETERS, returns: "i32 record count", scheduling: NativeSchedulingEffect::Continue, notes: "Returns the process-global bounded history count." },
    NativeOpcodeSpec { opcode: opcodes::SYS_APPEND_STRUCTURED_HISTORY_FIELDS, symbol: "Sys80_94_AppendStructuredHistoryFields", parameters: STRUCTURED_HISTORY_FIELDS_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "Allocates a native history node, validates 31/255-byte string limits, appends at the tail and evicts the oldest record at capacity." },
    NativeOpcodeSpec { opcode: opcodes::SYS_READ_STRUCTURED_HISTORY, symbol: "Sys80_95_ReadStructuredHistory", parameters: STRUCTURED_HISTORY_READ_PARAMETERS, returns: "boolean success (invalid index is fatal)", scheduling: NativeSchedulingEffect::Continue, notes: "Index zero selects the newest record; writes the target record layout without the extended field." },
    NativeOpcodeSpec { opcode: opcodes::SYS_APPEND_STRUCTURED_HISTORY_RECORD, symbol: "Sys80_96_AppendStructuredHistoryRecord", parameters: STRUCTURED_HISTORY_RECORD_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "Imports the target scalar/string record layout, including the optional extended text at byte 512." },
    NativeOpcodeSpec { opcode: opcodes::SYS_READ_STRUCTURED_HISTORY_EXTENDED, symbol: "Sys80_97_ReadStructuredHistoryExtended", parameters: STRUCTURED_HISTORY_READ_PARAMETERS, returns: "boolean success (invalid index is fatal)", scheduling: NativeSchedulingEffect::Continue, notes: "Same newest-first lookup as 0x95 and additionally copies the optional extended text." },
    NativeOpcodeSpec { opcode: opcodes::SYS_INDEXED_RECORD_OPEN, symbol: "Sys80_98_IndexedRecordOpen", parameters: INDEXED_RECORD_OPEN_PARAMETERS, returns: "status 0 or 2", scheduling: NativeSchedulingEffect::Continue, notes: "Creates a capacity-bounded fixed-record table and writes a monotonically allocated integer identifier." },
    NativeOpcodeSpec { opcode: opcodes::SYS_INDEXED_RECORD_CLOSE, symbol: "Sys80_99_IndexedRecordClose", parameters: INDEXED_RECORD_HANDLE_PARAMETERS, returns: "status 0 or 1", scheduling: NativeSchedulingEffect::Continue, notes: "Closes an indexed-record table; missing identifiers return one." },
    NativeOpcodeSpec { opcode: opcodes::SYS_INDEXED_RECORD_COUNT, symbol: "Sys80_9A_IndexedRecordCount", parameters: INDEXED_RECORD_COUNT_PARAMETERS, returns: "status 0 or 1", scheduling: NativeSchedulingEffect::Continue, notes: "Writes the current record count through the output pointer." },
    NativeOpcodeSpec { opcode: opcodes::SYS_INDEXED_RECORD_PUSH, symbol: "Sys80_9C_IndexedRecordPush", parameters: INDEXED_RECORD_PUSH_PARAMETERS, returns: "status 0 or 1", scheduling: NativeSchedulingEffect::Continue, notes: "Copies one fixed-size record to the front and truncates records beyond capacity." },
    NativeOpcodeSpec { opcode: opcodes::SYS_INDEXED_RECORD_LOAD, symbol: "Sys80_9D_IndexedRecordLoad", parameters: INDEXED_RECORD_LOAD_PARAMETERS, returns: "status 0..=2", scheduling: NativeSchedulingEffect::Continue, notes: "Loads one newest-first record; missing table is one and invalid index is two." },
    NativeOpcodeSpec { opcode: opcodes::SYS_INDEXED_RECORD_REMOVE, symbol: "Sys80_9E_IndexedRecordRemove", parameters: INDEXED_RECORD_REMOVE_PARAMETERS, returns: "status 0..=2", scheduling: NativeSchedulingEffect::Continue, notes: "Removes up to count records starting at a newest-first index." },
    NativeOpcodeSpec { opcode: opcodes::SYS_POLL_QUEUED_EVENT, symbol: "Sys80_A0_PollQueuedEvent", parameters: QUEUED_EVENT_POLL_PARAMETERS, returns: "boolean dequeued", scheduling: NativeSchedulingEffect::Continue, notes: "Pops the oldest process-global three-DWORD event into caller memory." },
    NativeOpcodeSpec { opcode: opcodes::SYS_POST_QUEUED_EVENT, symbol: "Sys80_A1_PostQueuedEvent", parameters: QUEUED_EVENT_POST_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "Queues [0,event_code,parameter] in the target critical-section protected FIFO." },
    NativeOpcodeSpec { opcode: opcodes::SYS_SET_REGISTERED_OBJECT_STATE, symbol: "Sys80_A8_SetRegisteredObjectState", parameters: REGISTERED_OBJECT_STATE_SET_PARAMETERS, returns: "boolean object found", scheduling: NativeSchedulingEffect::Continue, notes: "Looks up a DCIndProc registry entry and writes its state field at native offset +0x10." },
    NativeOpcodeSpec { opcode: opcodes::SYS_GET_REGISTERED_OBJECT_STATE, symbol: "Sys80_A9_GetRegisteredObjectState", parameters: REGISTERED_OBJECT_STATE_GET_PARAMETERS, returns: "boolean object found", scheduling: NativeSchedulingEffect::Continue, notes: "Looks up a registered object and writes its current state through the output pointer." },
    NativeOpcodeSpec { opcode: opcodes::SYS_QUEUE_REGISTERED_OBJECT_MESSAGE, symbol: "Sys80_AC_QueueRegisteredObjectMessage", parameters: REGISTERED_OBJECT_MESSAGE_PARAMETERS, returns: "boolean object found", scheduling: NativeSchedulingEffect::Continue, notes: "Returns zero only when registry lookup fails. For an existing object, a 1..=256 DWORD descriptor is queued; an invalid count queues nothing but still returns one, matching sub_48A250." },
    NativeOpcodeSpec { opcode: opcodes::SYS_SET_SYSTEM_MODE_FLAG, symbol: "Sys80_AF_SetSystemModeFlag", parameters: SYSTEM_MODE_PARAMETERS, returns: "boolean accepted", scheduling: NativeSchedulingEffect::Continue, notes: "Accepts only zero or one and stores the process-global mode queried by the System81 extension." },
    NativeOpcodeSpec { opcode: opcodes::SYS_CREATE_EXCLUSION_SECTION, symbol: "Sys80_B0_CreateExclusionSection", parameters: EXCLUSION_CREATE_PARAMETERS, returns: "boolean created", scheduling: NativeSchedulingEffect::Continue, notes: "Creates a named capacity-limited CExclusion section; duplicate names return zero." },
    NativeOpcodeSpec { opcode: opcodes::SYS_DELETE_EXCLUSION_SECTION, symbol: "Sys80_B1_DeleteExclusionSection", parameters: EXCLUSION_NAME_PARAMETERS, returns: "native status", scheduling: NativeSchedulingEffect::Continue, notes: "Returns 0x80000001 when missing and 0x80000002 while holders or waiters remain." },
    NativeOpcodeSpec { opcode: opcodes::SYS_WAIT_EXCLUSION_SECTION, symbol: "Sys80_B4_WaitExclusionSection", parameters: EXCLUSION_PRIORITY_PARAMETERS, returns: "deferred native status", scheduling: NativeSchedulingEffect::WaitProcedure, notes: "Installs CProcExclusion, queues the current thread by descending priority and resumes only when it is queue head and capacity is available." },
    NativeOpcodeSpec { opcode: opcodes::SYS_LEAVE_EXCLUSION_SECTION, symbol: "Sys80_B5_LeaveExclusionSection", parameters: EXCLUSION_NAME_PARAMETERS, returns: "native status", scheduling: NativeSchedulingEffect::Continue, notes: "Releases one holder entry for the current thread; missing/not-owner map to 0x80000001/0x80000003." },
    NativeOpcodeSpec { opcode: opcodes::SYS_QUERY_EXCLUSION_SECTION, symbol: "Sys80_B6_QueryExclusionSection", parameters: EXCLUSION_PRIORITY_PARAMETERS, returns: "native status", scheduling: NativeSchedulingEffect::Continue, notes: "Returns zero when capacity covers current holders plus all queued priorities greater than or equal to the request, otherwise 0x80000004." },
    NativeOpcodeSpec {
    ...
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_CONFIGURE_COMPACT_WAVE_TABLE, symbol: "Graph92_00_ConfigureCompactWaveTable", parameters: GRAPH92_CONFIGURE_COMPACT_WAVE_TABLE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "0x004854A0 -> sub_409AF0 fills one of eight wave slots with one sine cycle followed by zero rows in each block." },
    ...
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_OBJECT_SUPPRESSED, symbol: "Graph91_31_SetObjectSuppressed", parameters: GRAPH91_OBJECT_BOOLEAN_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_481960 reaches sub_41ADA0 and writes CDspObj+0x0C, which sub_41AE30 tests inversely before drawing." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_OBJECT_FIXED_POSITION, symbol: "Graph91_33_SetObjectFixedPosition", parameters: GRAPH91_OBJECT_FIXED_POSITION_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4819A0 invokes vtable+60. Base sub_41B370 consults +0x80/+0x84 first: when +0x80!=0 and (Z==0 or +0x84==1), X/Y are rounded as (v+0x8000)&0xFFFF0000 with 32-bit wrapping. It stores signed 16.16 X/Y/Z at +0x4C/+0x50/+0x54; if +0x7C!=0 it mirrors arithmetic X>>16/Y>>16 through vtable+0x28. Base construction sets +0x7C=1 and +0x80/+0x84=0/0; Sprite modes 5/6 and BackML clear +0x7C. The setter then eagerly propagates child base-vector deltas. sub_41B4C0 resolves one object by adding its three fixed-vector banks without traversing parents." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_OBJECT_SECONDARY_VECTOR, symbol: "Graph91_36_SetObjectSecondaryVector", parameters: GRAPH91_OBJECT_VECTOR_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4819F0 reaches sub_41B580 and stores signed 16.16 X/Y/Z at CDspObj+0x6C/+0x70/+0x74, propagates the bank to children, then reapplies the primary vector through vtable slot +68. sub_41B4C0 adds this bank to +0x4C/+0x50/+0x54 and +0x5C/+0x60/+0x64 before mode-5 projection." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_OBJECT_PRIMARY_VECTOR, symbol: "Graph91_37_SetObjectPrimaryVector", parameters: GRAPH91_OBJECT_VECTOR_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_481A40 invokes vtable slot +68; sub_41B520 stores signed 16.16 X/Y/Z at CDspObj+0x5C/+0x60/+0x64 and propagates them to children. sub_41B4C0 adds this bank to +0x4C/+0x50/+0x54 and +0x6C/+0x70/+0x74 before mode-5 projection." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_GET_OBJECT_PROPERTY, symbol: "Graph91_38_GetObjectProperty", parameters: GRAPH91_GET_OBJECT_PROPERTY_PARAMETERS, returns: "void; target reports invalid object/output or unsupported parameter", scheduling: NativeSchedulingEffect::Continue, notes: "sub_481A90 converts the first BP argument and invokes the object vtable +96 getter with the requested parameter number." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_GET_OBJECT_COMPOSITE_POSITION, symbol: "Graph91_3D_GetObjectCompositePosition", parameters: GRAPH91_GET_OBJECT_POSITION_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_481B50 converts the first BP argument and sub_41B260 writes exactly two resolved coordinate DWORDs." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_ATTACH_CHILD_OBJECT, symbol: "Graph91_3E_AttachChildObject", parameters: GRAPH91_ATTACH_OBJECT_PARAMETERS, returns: "void; target emits script errors for invalid/self/already-owned objects", scheduling: NativeSchedulingEffect::Continue, notes: "sub_481B90 validates master and slave then sub_41AB40 creates a 0x10 member record containing slave/local-X/local-Y/next. This is eager state propagation, not a renderer-time scene-graph transform. In the ordinary branch the master raw CDspObj +0x30/+0x34 position plus the supplied local offsets is written immediately to the slave. Only when both objects' vtable+0x70 reports the projected Sprite coordinate domain (modes 5/6) does the fixed branch use the master's raw base 16.16 +0x4C/+0x50/+0x54 vector, add local X/Y in that domain, inherit Z, and write the slave through vtable+0x3C. Detach preserves the last materialized slave coordinates." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_DETACH_CHILD_OBJECT, symbol: "Graph91_3F_DetachChildObject", parameters: GRAPH91_DETACH_OBJECT_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_481D30 calls sub_41AC40 and requires the slave to be a current dependent of the supplied master." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_INITIALIZE_MULTILAYER_BACKGROUND, symbol: "Graph91_40_InitializeMultiLayerBackground", parameters: GRAPH91_MULTILAYER_INITIALIZE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_481D90 switches the active background to CDspObjBackML mode 12, configures/enables layer zero, selects it and sets the background position." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SELECT_MULTILAYER_LAYER, symbol: "Graph91_41_SelectMultiLayer", parameters: GRAPH91_MULTILAYER_INDEX_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_481EC0 requires mode 12 and stores one of eight current-layer indices." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_MULTILAYER_LAYER_ENABLED, symbol: "Graph91_42_SetMultiLayerEnabled", parameters: GRAPH91_MULTILAYER_ENABLED_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_481F30 writes the layer-record enable field and requests redraw when successful." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_MULTILAYER_LAYER_POSITION, symbol: "Graph91_43_SetMultiLayerPosition", parameters: GRAPH91_MULTILAYER_PAIR_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_481FB0 calls sub_41DC80 and stores signed 16.16 X/Y at record offsets +8/+12, with target pixel snapping when active." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_MULTILAYER_LAYER_BLEND_MODE, symbol: "Graph91_44_SetMultiLayerBlendMode", parameters: GRAPH91_MULTILAYER_SCALAR_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_482040 validates the native blend selector and writes record offset +16." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_MULTILAYER_LAYER_BLEND_PARAMETER, symbol: "Graph91_45_SetMultiLayerBlendParameter", parameters: GRAPH91_MULTILAYER_SCALAR_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4820D0 validates 0..=256 and writes record offset +20." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_MULTILAYER_LAYER_BITMAP, symbol: "Graph91_46_SetMultiLayerBitmap", parameters: GRAPH91_MULTILAYER_BITMAP_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_482160 calls sub_41DDC0; -1 clears the layer, otherwise bitmap generation and source X/Y are stored and source deltas reset." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_MULTILAYER_LAYER_TRANSFORM, symbol: "Graph91_47_SetMultiLayerTransform", parameters: GRAPH91_MULTILAYER_TRANSFORM_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_482230 calls sub_41DE80, rejects zero scales, stores rotation/scale/transparency and clears all transform deltas." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_MULTILAYER_LAYER_AUXILIARY_PAIR, symbol: "Graph91_48_SetMultiLayerAuxiliaryPair", parameters: GRAPH91_MULTILAYER_PAIR_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_482310 calls sub_41DEF0 and writes the two record DWORDs at offsets +52/+56." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_MULTILAYER_LAYER_SOURCE_VELOCITY, symbol: "Graph91_49_SetMultiLayerSourceVelocity", parameters: GRAPH91_MULTILAYER_PAIR_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4823A0 calls sub_41DF20 and writes per-frame source-coordinate increments at offsets +60/+64." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_MULTILAYER_LAYER_TRANSFORM_VELOCITY, symbol: "Graph91_4A_SetMultiLayerTransformVelocity", parameters: GRAPH91_MULTILAYER_TRANSFORM_VELOCITY_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_482430 writes the rotation increment at +68 and scale increments at +72/+76." },

    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_SPRITE_RELATION, symbol: "Graph91_55_SetSpriteRelation", parameters: GRAPH91_SPRITE_RELATION_PARAMETERS, returns: "i32 status", scheduling: NativeSchedulingEffect::Continue, notes: "0x004824D0 resolves one required and one optional CDspObjSprite then calls sub_428CD0; zero clears the relation." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CREATE_EFFECTOR, symbol: "Graph91_60_CreateEffector", parameters: NO_PARAMETERS, returns: "0x91000000 tagged handle or 0", scheduling: NativeSchedulingEffect::Continue, notes: "sub_43F290 allocates one of eight CDspObjEffector slots constructed by sub_41FCA0 with CDspObj sort_class=6 and a per-registry monotonic construction index at CDspObj+0x20; the index is independent of the reusable tagged slot." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_RELEASE_EFFECTOR, symbol: "Graph91_61_ReleaseEffector", parameters: GRAPH91_EFFECTOR_HANDLE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_43F390 releases a validated 0x91000000-tagged CDspObjEffector." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_EFFECTOR_ENABLED, symbol: "Graph91_64_SetEffectorEnabled", parameters: GRAPH91_EFFECTOR_ENABLED_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_43F730 forwards the native enable gate to the effector object." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CONFIGURE_DUAL_VECTOR_EFFECTOR, symbol: "Graph91_65_ConfigureDualVectorMapEffector", parameters: GRAPH91_DUAL_VECTOR_EFFECTOR_PARAMETERS, returns: "void; target reports map validation errors", scheduling: NativeSchedulingEffect::Continue, notes: "sub_43F410 -> sub_420110 selects effector mode zero and validates one or two screen-sized format-4 maps." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CONFIGURE_GRADIENT_EFFECTOR, symbol: "Graph91_66_ConfigureGradientEffector", parameters: GRAPH91_GRADIENT_EFFECTOR_PARAMETERS, returns: "void; target rejects gradient types outside 0..=1", scheduling: NativeSchedulingEffect::Continue, notes: "sub_43F4E0 -> sub_420250 selects effector mode one." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CONFIGURE_RIPPLE_EFFECTOR, symbol: "Graph91_67_ConfigureRippleEffector", parameters: GRAPH91_RIPPLE_EFFECTOR_PARAMETERS, returns: "void; target validates map, maximum distance and ripple registration", scheduling: NativeSchedulingEffect::Continue, notes: "sub_43F560 -> sub_4202A0 selects mode two and requires a screen-sized format-6 vector/distance map." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CONFIGURE_TRANSFORM_EFFECTOR, symbol: "Graph91_68_ConfigureTransformEffector", parameters: GRAPH91_TRANSFORM_EFFECTOR_PARAMETERS, returns: "void; target rejects zero scales", scheduling: NativeSchedulingEffect::Continue, notes: "sub_43F640 -> sub_420410 selects mode three and stores base position, rotation, scales and composition fields." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CONFIGURE_SURFACE_EFFECTOR, symbol: "Graph91_69_ConfigureSurfaceEffector", parameters: GRAPH91_SURFACE_EFFECTOR_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_43F6D0 -> sub_4204D0 selects mode four and clears the internal destination surface." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CREATE_LANDSCAPE, symbol: "Graph91_70_CreateLandscape", parameters: GRAPH91_CREATE_LANDSCAPE_PARAMETERS, returns: "0xA1000000 tagged handle or 0", scheduling: NativeSchedulingEffect::Continue, notes: "sub_43FC50 allocates one of four CDspObjLandscape slots; sub_421780 applies native defaults for zero/odd geometry fields and constructs CDspObj with sort_class=1 plus a per-registry monotonic construction index at +0x20, independent of the reusable tagged slot." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_RELEASE_LANDSCAPE, symbol: "Graph91_71_ReleaseLandscape", parameters: GRAPH91_LANDSCAPE_HANDLE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_43FD70 releases a validated 0xA1000000-tagged CDspObjLandscape." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_HIT_TEST_LANDSCAPE, symbol: "Graph91_73_HitTestLandscapeAtPointer", parameters: GRAPH91_LANDSCAPE_HIT_TEST_PARAMETERS, returns: "i32 boolean", scheduling: NativeSchedulingEffect::Continue, notes: "sub_482DA0 obtains the current cursor, subtracts object position and sub_422920 writes the hit line/column pair." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_LANDSCAPE_ENABLED, symbol: "Graph91_74_SetLandscapeEnabled", parameters: GRAPH91_LANDSCAPE_ENABLED_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "0x00482E10 invokes the base CDspObj enable setter." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CONFIGURE_LANDSCAPE_OBJECT, symbol: "Graph91_75_ConfigureLandscapeObject", parameters: GRAPH91_LANDSCAPE_OBJECT_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_422AF0 sets object position, blend field, composition value and priority." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_LANDSCAPE_CELL_SILHOUETTE, symbol: "Graph91_76_SetLandscapeCellSilhouette", parameters: GRAPH91_LANDSCAPE_SILHOUETTE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_422730 validates configured map coordinates, silhouette type <=2 and level <=256, then updates the 68-byte cell record." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CONFIGURE_LANDSCAPE_PARTS, symbol: "Graph91_78_ConfigureLandscapePartsAndColumns", parameters: GRAPH91_LANDSCAPE_PARTS_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_421930 consumes five-DWORD part descriptors and 34-DWORD pillar/column descriptors from BP memory." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CONFIGURE_LANDSCAPE_MAP, symbol: "Graph91_79_ConfigureLandscapeMap", parameters: GRAPH91_LANDSCAPE_MAP_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_421E60 validates dimensions 1..=256 and consumes width*height i32 pillar/column indices." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CONFIGURE_LANDSCAPE_GUIDES, symbol: "Graph91_7A_ConfigureLandscapeGuides", parameters: GRAPH91_LANDSCAPE_GUIDES_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4224D0 consumes five-DWORD guide-image descriptors." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_LANDSCAPE_CELL_GUIDES, symbol: "Graph91_7B_SetLandscapeCellGuides", parameters: GRAPH91_LANDSCAPE_CELL_GUIDES_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_422680 applies one of four guide layers to each BP-provided column/line pair." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_COPY_LANDSCAPE_PART, symbol: "Graph91_7C_CopyLandscapePart", parameters: GRAPH91_LANDSCAPE_PART_COPY_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_422050 copies source part image state only when destination geometry/format is compatible." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_LANDSCAPE_CELL_COLUMN, symbol: "Graph91_7D_SetLandscapeMapCellColumn", parameters: GRAPH91_LANDSCAPE_CELL_COLUMN_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4222A0 changes one cell pillar/column index and updates affected neighboring records." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_GET_LANDSCAPE_CELL_VALUE, symbol: "Graph91_7E_GetLandscapeCellValue", parameters: GRAPH91_LANDSCAPE_CELL_VALUE_PARAMETERS, returns: "i32 boolean", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4228B0 writes a zero-extended WORD from cell-record offset +22 through the BP pointer." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_COPY_LANDSCAPE_CELL_IMAGE, symbol: "Graph91_7F_CopyLandscapeCellImage", parameters: GRAPH91_LANDSCAPE_CELL_IMAGE_PARAMETERS, returns: "i32 boolean", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4227C0 copies the selected cell-part image into an existing destination bitmap." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CONFIGURE_WINDOW_FONT, symbol: "Graph91_88_ConfigureWindowFont", parameters: GRAPH91_WINDOW_FONT_PARAMETERS, returns: "void; target reports native font/window validation errors", scheduling: NativeSchedulingEffect::Continue, notes: "0x004840A0 reaches sub_4409C0 and stores the recovered CDspObjWindow font fields at +0x354..+0x364." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_WINDOW_LINE_SPACING, symbol: "Graph91_89_SetWindowLineSpacingPercent", parameters: GRAPH91_WINDOW_LINE_SPACING_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "0x004841D0 validates 0..=800 and stores CDspObjWindow+0x360." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_WINDOW_MESSAGE_VARIANT, symbol: "Graph91_8A_SetWindowMessageVariant", parameters: MESSAGE_VARIANT_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "0x00484250 stores CDspObjWindow+0x370; value one selects the ExVE path where consumed." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_WINDOW_TEXT_LAYOUT_MODE, symbol: "Graph91_8B_SetWindowTextLayoutMode", parameters: GRAPH91_WINDOW_LAYOUT_MODE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "0x004842D0 validates 0..=2 and stores CDspObjWindow+0x374." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_WINDOW_TEXT_CURSOR, symbol: "Graph91_8C_SetWindowTextCursor", parameters: GRAPH91_WINDOW_CURSOR_SET_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "0x00484350 stores cursor X/Y at CDspObjWindow+0x368/+0x36C." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_GET_WINDOW_TEXT_CURSOR, symbol: "Graph91_8D_GetWindowTextCursor", parameters: GRAPH91_WINDOW_HANDLE_PARAMETERS, returns: "three outputs: valid, cursor_x, cursor_y", scheduling: NativeSchedulingEffect::Continue, notes: "0x004843A0 returns the window-exists flag followed by both cursor coordinates." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_IS_WINDOW_TEXT_CURSOR_AT_BOUNDARY, symbol: "Graph91_8E_IsWindowTextCursorAtBoundary", parameters: GRAPH91_WINDOW_HANDLE_PARAMETERS, returns: "i32 boolean", scheduling: NativeSchedulingEffect::Continue, notes: "sub_42C740 compares one cursor axis for exact equality with valid-rectangle origin plus dword_565BDC, selected by the message variant." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_START_EXTENDED_MESSAGE, symbol: "Graph91_90_StartExtendedMessage", parameters: GRAPH91_MESSAGE_EX_PARAMETERS, returns: "native procedure status 2", scheduling: NativeSchedulingEffect::WaitProcedure, notes: "0x00484430 installs CProcDspMsgEx through sub_4911E0/sub_491220. The inherited CProcDspMsg constructor/destructor owns the exact +0x78 input-scope registration lifecycle; byte 0x0A remains an immediate line-layout control rather than one glyph interval." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_RENDER_WINDOW_TEXT, symbol: "Graph91_91_RenderWindowText", parameters: GRAPH91_RENDER_TEXT_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "0x004844F0 performs the immediate window text render/update path." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_START_EXTENDED_MESSAGE_WITH_OPTION, symbol: "Graph91_92_StartExtendedMessageWithOption", parameters: GRAPH91_MESSAGE_EX_OPTION_PARAMETERS, returns: "native procedure status 2", scheduling: NativeSchedulingEffect::WaitProcedure, notes: "0x00484580 installs CProcDspMsgEx and supplies one additional constructor option." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_RENDER_WINDOW_TEXT_WITH_STYLE_MODE, symbol: "Graph91_93_RenderWindowTextWithStyleMode", parameters: GRAPH91_RENDER_TEXT_STYLE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "0x00484650 selects default versus zeroed style state before immediate rendering." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_UPDATE_TEXT_SUBSTITUTION, symbol: "Graph91_94_UpdateTextSubstitution", parameters: GRAPH91_TEXT_SUBSTITUTION_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "0x00484710 adds, removes, or clears the process-global substitution dictionary according to nullable arguments." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_COUNT_TEXT_SUBSTITUTION_MATCHES, symbol: "Graph91_95_CountTextSubstitutionMatches", parameters: GRAPH91_TEXT_SUBSTITUTION_COUNT_PARAMETERS, returns: "i32 match count", scheduling: NativeSchedulingEffect::Continue, notes: "0x00484740 scans the source and returns only the match count; its other converted argument is ignored and no output pointer is written." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_REGISTER_TEXT_SUBSTITUTION_RECORDS, symbol: "Graph91_96_RegisterTextSubstitutionRecords", parameters: GRAPH91_TEXT_SUBSTITUTION_RECORDS_PARAMETERS, returns: "i32 boolean", scheduling: NativeSchedulingEffect::Continue, notes: "0x00484770 parses newline-delimited base\\reading records and registers them." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_PERSISTENT_TEXT_STYLE, symbol: "Graph91_97_SetPersistentTextStyle", parameters: GRAPH91_TEXT_STYLE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "0x004847A0 resolves the font and calls sub_434440 with four script fields plus two target -1 defaults." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CONFIGURE_TEXT_LAYOUT_GLOBALS, symbol: "Graph91_98_ConfigureTextLayoutGlobals", parameters: GRAPH91_TEXT_LAYOUT_GLOBAL_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "0x00484800 maps all six BP arguments to the recovered globals, validates font_percent 25..=100 and boundary_offset >=0, and normalizes a zero denominator to one." },
    ...
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_START_DIRECTSHOW_MOVIE, symbol: "Graph91_F1_StartDirectShowMovie", parameters: GRAPH91_MOVIE_START_PARAMETERS, returns: "i32 status 0/1/4", scheduling: NativeSchedulingEffect::Continue, notes: "0x00485100 -> sub_408430/sub_44D110 prepares/runs the media graph and writes duration milliseconds through the first source argument." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CLOSE_DIRECTSHOW_MOVIE, symbol: "Graph91_F2_CloseDirectShowMovie", parameters: GRAPH91_MOVIE_HANDLE_PARAMETERS, returns: "i32 status 0/1/4", scheduling: NativeSchedulingEffect::Continue, notes: "0x00485170 -> sub_4084B0 stops and releases the renderer, then clears the slot's async object." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_SET_DIRECTSHOW_MOVIE_PAUSED, symbol: "Graph91_F3_SetDirectShowMoviePaused", parameters: GRAPH91_MOVIE_PAUSE_PARAMETERS, returns: "i32 status 0/1/4", scheduling: NativeSchedulingEffect::Continue, notes: "0x004851D0 -> sub_408550/sub_44D240 resumes for zero and pauses for nonzero after preparation." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CREATE_FLASH_CONTROL, symbol: "Graph91_F4_CreateFlashControl", parameters: GRAPH91_FLASH_CREATE_PARAMETERS, returns: "i32 status 0/1/2/4", scheduling: NativeSchedulingEffect::Continue, notes: "0x00485240 constructs an ATL-hosted Shockwave Flash ActiveX control, assigns the SWF path and creates a type-1 destination bitmap. The portable backend loads the same SWF bytes into an embedded Ruffle offscreen wgpu player; exact ATL COM, audio and host callback behavior remains partial." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_START_FLASH_CONTROL, symbol: "Graph91_F5_StartFlashControl", parameters: GRAPH91_MOVIE_HANDLE_PARAMETERS, returns: "i32 status 0/1/2/4", scheduling: NativeSchedulingEffect::Continue, notes: "0x00485300 sends command 32 to the SCFlashControl. The portable backend resumes the embedded Ruffle player and advances it from the engine native-tick clock." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_CAPTURE_AND_RELEASE_FLASH_CONTROL, symbol: "Graph91_F6_CaptureAndReleaseFlashControl", parameters: GRAPH91_MOVIE_HANDLE_PARAMETERS, returns: "i32 status 0/1/4", scheduling: NativeSchedulingEffect::Continue, notes: "0x00485380 sends command 33, copies the ActiveX DIB into the destination bitmap, then destroys and clears the control. The portable backend renders and captures the final Ruffle TextureTarget frame before removing the player." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH91_GET_MOVIE_POSITION, symbol: "Graph91_F7_GetMoviePosition", parameters: GRAPH91_MOVIE_POSITION_PARAMETERS, returns: "i32 boolean", scheduling: NativeSchedulingEffect::Continue, notes: "0x004853F0 -> sub_407FB0 writes the live DirectShow position, cached slot position, or -1 according to the slot state." },
    NativeOpcodeSpec {
    ...
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_SET_TEXT_OBJECT_VALUE, symbol: "Graph92_88_SetTextObjectValue", parameters: GRAPH92_TEXT_OBJECT_VALUE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4860D0 -> sub_462CB0 -> sub_440B00 resolves the text object, stores the scalar at native offset +348 through sub_42B3B0, invalidates it, and invokes its update virtuals." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_COMPOSITE_RESOURCE_INTO_TEXT_OBJECT, symbol: "Graph92_89_CompositeResourceIntoTextObject", parameters: GRAPH92_TEXT_OBJECT_COMPOSITE_PARAMETERS, returns: "void or script error", scheduling: NativeSchedulingEffect::Continue, notes: "sub_486110 maps target object/resource/rectangle composition statuses. The portable renderer preserves the object/resource boundary but not the proprietary native rectangle kernel." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_SET_TEXT_OBJECT_AUXILIARY_VALUE, symbol: "Graph92_8A_SetTextObjectAuxiliaryValue", parameters: GRAPH92_TEXT_OBJECT_VALUE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_486220 -> sub_440B50 -> sub_42B3C0 writes the recovered object substructure at +356 when the text object is initialized." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_SET_TEXT_OBJECT_UPDATE_FLAG, symbol: "Graph92_8C_SetTextObjectUpdateFlag", parameters: GRAPH92_TEXT_OBJECT_VALUE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_486260 -> sub_440DC0 stores the scalar at native offset +380, invalidates the object, and invokes update virtuals." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_APPLY_EFFECT_RESOURCE, symbol: "Graph92_8D_ApplyEffectResource", parameters: GRAPH92_TEXT_OBJECT_COMPOSITE_PARAMETERS, returns: "void or script error", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4862A0 -> sub_440E10 -> sub_42B560/sub_42B5B0 validates a resource rectangle and applies it to the text object with target status mapping." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_RESET_TEXT_OBJECT, symbol: "Graph92_8E_ResetTextObject", parameters: GRAPH92_RESET_TEXT_OBJECT_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_440F80 clears cursor/layout state, the object text bitmap, eight state records, and allocated child text entries before invoking update virtuals." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_START_STYLED_MESSAGE, symbol: "Graph92_90_StartStyledMessage", parameters: GRAPH92_START_STYLED_MESSAGE_PARAMETERS, returns: "native procedure status 2", scheduling: NativeSchedulingEffect::WaitProcedure, notes: "sub_4863E0 builds a five-DWORD style tuple and installs CProcDspMsgEx or CProcDspMsgExVE through sub_491220. The inherited CProcDspMsg owns exact +0x78 input-scope registration/drain/unregistration. Raw/newline <cr> controls are processed without consuming Graph90:94 glyph cadence." },
    ...
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_OPEN_GLOBAL_DIRECTSHOW_MOVIE, symbol: "Graph92_F0_OpenGlobalDirectShowMovie", parameters: GRAPH92_OPEN_GLOBAL_MOVIE_PARAMETERS, returns: "duration milliseconds or 0", scheduling: NativeSchedulingEffect::Continue, notes: "sub_486B80 validates positive width/height, converts archive/resource strings, and calls sub_48F270 to open the process-global movie and return its duration." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_LOAD_BURIKO_MOVIE_RESOURCE, symbol: "Graph92_F1_LoadBurikoMovieResource", parameters: GRAPH92_LOAD_BURIKO_MOVIE_PARAMETERS, returns: "deferred loader status", scheduling: NativeSchedulingEffect::WaitProcedure, notes: "sub_486C40 pops mode/resource/archive/metadata_out/handle_out. Mode zero installs DCProcLoadBurikoMV; nonzero installs DCProcLoadBMVHeader. The VM owns the handle and five-DWORD metadata writes." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_OPEN_BITMAP_DIRECTSHOW_MOVIE, symbol: "Graph92_F2_OpenBitmapDirectShowMovie", parameters: GRAPH92_OPEN_BITMAP_MOVIE_PARAMETERS, returns: "status 0/1/2/3/4", scheduling: NativeSchedulingEffect::Continue, notes: "sub_486D30 converts archive/resource and calls sub_48F7B0/sub_4083F0 to construct DCMovieRenderer in the selected bitmap slot with loop and volume options." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_SEEK_BITMAP_DIRECTSHOW_MOVIE, symbol: "Graph92_F4_SeekBitmapDirectShowMovie", parameters: GRAPH92_SEEK_BITMAP_MOVIE_PARAMETERS, returns: "status 0/1/4/5", scheduling: NativeSchedulingEffect::Continue, notes: "sub_486E10 -> sub_4085A0 resolves the slot renderer and forwards the absolute millisecond position to sub_44D2C0." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_GET_BITMAP_DIRECTSHOW_MOVIE_POSITION, symbol: "Graph92_F5_GetBitmapDirectShowMoviePosition", parameters: GRAPH92_GET_BITMAP_MOVIE_POSITION_PARAMETERS, returns: "status 0/1/4", scheduling: NativeSchedulingEffect::Continue, notes: "sub_486EA0 -> sub_408600 writes the current DirectShow position through the first source argument. The VM owns the DWORD write." },
    NativeOpcodeSpec { opcode: opcodes::GRAPH92_SET_BITMAP_DIRECTSHOW_MOVIE_VOLUME, symbol: "Graph92_F6_SetBitmapDirectShowMovieVolume", parameters: GRAPH92_SET_BITMAP_MOVIE_VOLUME_PARAMETERS, returns: "status 0/1/3/4", scheduling: NativeSchedulingEffect::Continue, notes: "sub_486F10 -> sub_408650 resolves the slot renderer and applies the inclusive 0..=128 native volume." },
    // SystemA0 target sound dispatcher.
    NativeOpcodeSpec { opcode: opcodes::SOUND_QUERY_CHANNEL_COUNT, symbol: "SoundA0_00_QueryChannelCount", parameters: NO_PARAMETERS, returns: "20", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487010 returns the target engine's fixed software-audio channel count." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_SET_BGM_PRIMARY_VOLUME, symbol: "SoundA0_08_SetBgmPrimaryVolume", parameters: SOUND_CHANNEL_VOLUME_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487030 -> sub_493B60 updates the first independent BGM gain bank." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_SET_SE_PRIMARY_VOLUME, symbol: "SoundA0_09_SetSePrimaryVolume", parameters: SOUND_CHANNEL_VOLUME_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487070 -> sub_493B90 updates the first independent SE gain bank." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_LOAD_BGM_FILE, symbol: "SoundA0_10_LoadBgmFile", parameters: SOUND_BGM_FILE_LOAD_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4870B0 -> sub_493C00 loads one resident BGM buffer without starting playback." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_LOAD_BGM_ARCHIVE, symbol: "SoundA0_11_LoadBgmArchive", parameters: SOUND_BGM_ARCHIVE_LOAD_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487180 -> sub_493DB0 loads one archive-backed resident BGM buffer and applies initial volume/pan; A0:14 starts it." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_LOAD_BGM_PAIR, symbol: "SoundA0_12_LoadBgmPair", parameters: SOUND_BGM_PAIR_LOAD_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487280 -> sub_4940D0 loads an intro/loop stream pair without starting playback." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_CONTROL_BGM, symbol: "SoundA0_14_ControlBgm", parameters: SOUND_BGM_CONTROL_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4873C0 -> sub_4A3150 pauses on zero and starts/resumes on non-zero." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_QUERY_BGM_STATE, symbol: "SoundA0_15_QueryBgmState", parameters: SOUND_BGM_QUERY_PARAMETERS, returns: "status", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487400 writes one buffer state/position DWORD through the second source argument; VM mediation owns the pointer write." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_SET_BGM_VOLUME, symbol: "SoundA0_16_SetBgmVolume", parameters: SOUND_BGM_FADE_VOLUME_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487450 updates per-channel BGM gain and schedules the requested transition." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_SET_BGM_PAN, symbol: "SoundA0_17_SetBgmPan", parameters: SOUND_CHANNEL_PAN_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4874A0 maps native pan 0..128 around center 64." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_FADE_BGM_TO_FULL, symbol: "SoundA0_18_FadeBgmToFull", parameters: SOUND_CHANNEL_DURATION_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4874E0 schedules the independent BGM fade envelope toward 128." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_FADE_BGM_TO_SILENCE, symbol: "SoundA0_19_FadeBgmToSilence", parameters: SOUND_CHANNEL_DURATION_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487520 schedules the independent BGM fade envelope toward zero." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_SET_BGM_SECONDARY_VOLUME, symbol: "SoundA0_1C_SetBgmSecondaryVolume", parameters: SOUND_CHANNEL_VOLUME_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487560 -> sub_4A2BB0 updates the second independent BGM gain bank." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_LOAD_SE, symbol: "SoundA0_20_LoadSe", parameters: SOUND_SE_LOAD_PARAMETERS, returns: "deferred load status", scheduling: NativeSchedulingEffect::WaitProcedure, notes: "sub_4875A0 constructs CProcLoadSound for one archive-backed resident SE slot." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_LOAD_SE_SCALED, symbol: "SoundA0_21_LoadSeScaled", parameters: SOUND_SE_LOAD_SCALED_PARAMETERS, returns: "deferred load status", scheduling: NativeSchedulingEffect::WaitProcedure, notes: "sub_487650 constructs CProcLoadSound with decoder gain and an opaque native start parameter." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_RELEASE_SE, symbol: "SoundA0_22_ReleaseSe", parameters: SOUND_CHANNEL_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487740 releases the resident sound buffer for one SE channel." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_LOAD_SE_DOUBLE_RATE, symbol: "SoundA0_23_LoadSeDoubleRate", parameters: SOUND_SE_LOAD_SCALED_PARAMETERS, returns: "deferred load status", scheduling: NativeSchedulingEffect::WaitProcedure, notes: "sub_487770 constructs CProcLoadSound with fixed 2.0 playback-rate scaling." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_PLAY_SE, symbol: "SoundA0_24_PlaySe", parameters: SOUND_SE_PLAY_PARAMETERS, returns: "playback position/status", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487860 -> sub_4943A0 starts a resident SE buffer with per-play volume and pan." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_STOP_SE, symbol: "SoundA0_25_StopSe", parameters: SOUND_CHANNEL_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4878F0 stops one SE playback without releasing its resident buffer." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_FADE_SE_TO_SILENCE, symbol: "SoundA0_26_FadeSeToSilence", parameters: SOUND_CHANNEL_DURATION_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487920 schedules the independent SE fade envelope toward zero." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_LOAD_SE_CUSTOM_RATE, symbol: "SoundA0_27_LoadSeCustomRate", parameters: SOUND_SE_LOAD_CUSTOM_PARAMETERS, returns: "deferred load status", scheduling: NativeSchedulingEffect::WaitProcedure, notes: "sub_487960 constructs CProcLoadSound with explicit decoder gain, playback rate, and opaque native start parameter." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_REGISTER_SE_MEMORY, symbol: "SoundA0_28_RegisterSeMemory", parameters: SOUND_SE_REGISTER_PARAMETERS, returns: "deferred registration status", scheduling: NativeSchedulingEffect::WaitProcedure, notes: "sub_487A70 -> sub_452530 constructs DCProcRgstrSound for memory-backed sound registration." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_SET_SE_SECONDARY_VOLUME, symbol: "SoundA0_2C_SetSeSecondaryVolume", parameters: SOUND_CHANNEL_VOLUME_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487B60 -> sub_4A2890 updates the second independent SE gain bank." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_GET_SE_POSITION, symbol: "SoundA0_2F_GetSePosition", parameters: SOUND_POSITION_QUERY_PARAMETERS, returns: "position/status", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487BA0 queries the current resident SE playback position." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_OPEN_CD_AUDIO, symbol: "SoundA0_80_OpenCdAudio", parameters: NO_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487BE0 -> sub_48D640 opens the MCI CD-audio device and selects TMSF; portable runtime opens a real virtual CD-DA track set from loose or packed audio assets." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_CLOSE_CD_AUDIO, symbol: "SoundA0_81_CloseCdAudio", parameters: NO_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487BF0 -> sub_48D7C0 closes the CD-audio device; portable runtime stops the dedicated channel and releases its discovered track set." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_PLAY_CD_TRACK, symbol: "SoundA0_84_PlayCdTrack", parameters: SOUND_CD_PLAY_PARAMETERS, returns: "status", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487C00 -> sub_48D850 sends MCI_PLAY from track to track+1 with optional notify; portable runtime plays the matching one-based virtual CD-DA asset on a dedicated channel." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_STOP_CD_AUDIO, symbol: "SoundA0_85_StopCdAudio", parameters: NO_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487C40 -> sub_48D8F0 stops an open CD-audio device; portable runtime stops the dedicated virtual CD-DA channel and transitions to target mode 3." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_QUERY_CD_MODE, symbol: "SoundA0_86_QueryCdMode", parameters: SOUND_CD_QUERY_PARAMETERS, returns: "zero or one", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487C50 -> sub_48D910 maps MCI modes 524..530 to target values 0,3,2,6,1,4,5; VM mediation writes the virtual device mode and returns one only while the device is open." },
    NativeOpcodeSpec { opcode: opcodes::SOUND_PLAY_WAVE_ASYNC, symbol: "SoundA0_C0_PlayWaveAsync", parameters: SOUND_WAVE_PLAY_PARAMETERS, returns: "BOOL", scheduling: NativeSchedulingEffect::Continue, notes: "sub_487C80 calls PlaySoundA(path, hInstance, 0x22003); portable runtime maps this to a dedicated asynchronous sound channel." },


    // SystemB0 target-confirmed user/window/input/font handlers.
    NativeOpcodeSpec { opcode: opcodes::USER_DRAW_BITMAP_TO_WINDOW, symbol: "UserB0_00_DrawBitmapToWindow", parameters: USER_DRAW_BITMAP_TO_WINDOW_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478030 resolves the bitmap descriptor and draws it directly to the parent window HDC." },
    NativeOpcodeSpec { opcode: opcodes::USER_CENTER_MAIN_WINDOW, symbol: "UserB0_02_CenterMainWindow", parameters: NO_PARAMETERS, returns: "boolean success", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4780C0 -> sub_461690 centers the main window when the engine is not fullscreen." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_MAIN_WINDOW_POSITION, symbol: "UserB0_03_SetMainWindowPosition", parameters: USER_SET_MAIN_WINDOW_POSITION_PARAMETERS, returns: "boolean accepted", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4780E0 validates the requested position against the monitor work area before SetWindowPos." },
    NativeOpcodeSpec { opcode: opcodes::USER_BIND_CURSOR_OBJECT, symbol: "UserB0_04_BindCursorObject", parameters: USER_BIND_CURSOR_OBJECT_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478170 -> sub_48EBD0 binds one display object to the host cursor and manages native cursor visibility." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_CURSOR_IDLE_TIMEOUT, symbol: "UserB0_05_SetCursorIdleTimeout", parameters: USER_SET_CURSOR_IDLE_TIMEOUT_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4781C0 -> sub_48E850 configures the target pointer-idle visibility timer." },
    NativeOpcodeSpec { opcode: opcodes::USER_QUERY_CURSOR_VISIBLE, symbol: "UserB0_06_QueryCursorVisible", parameters: NO_PARAMETERS, returns: "previous cursor visibility latch", scheduling: NativeSchedulingEffect::Continue, notes: "Target sub_4781E0/sub_48EE20 pushes the old dword_503E74 value. When no cursor object is bound and that old value is nonzero, it then refreshes the latch from the inverse GetCursorInfo suppression bit for the next call." },
    NativeOpcodeSpec { opcode: opcodes::USER_SHAKE_SCREEN, symbol: "UserB0_08_ShakeScreen", parameters: USER_SHAKE_SCREEN_PARAMETERS, returns: "no deferred values", scheduling: NativeSchedulingEffect::WaitProcedure, notes: "sub_478200 constructs CProcShakeScreen; sub_43CDD0 uses axis/diagonal triangular motion for modes 0..2 and CRT-random endpoint interpolation for mode 3, then restores zero offset on completion. The procedure never calls sub_4450D0." },
    NativeOpcodeSpec { opcode: opcodes::USER_CREATE_DEBUG_WINDOW, symbol: "UserB0_10_CreateDebugWindow", parameters: USER_CREATE_DEBUG_WINDOW_PARAMETERS, returns: "tagged debug-window handle", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4783D0 -> sub_42FB00 allocates one of eight 0xFF000000|slot auxiliary drawing windows." },
    NativeOpcodeSpec { opcode: opcodes::USER_CLOSE_DEBUG_WINDOW, symbol: "UserB0_11_CloseDebugWindow", parameters: USER_CLOSE_DEBUG_WINDOW_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4784A0 -> sub_42FC40 releases one auxiliary window slot." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_DEBUG_WINDOW_VISIBLE, symbol: "UserB0_14_SetDebugWindowVisible", parameters: USER_SET_DEBUG_WINDOW_VISIBLE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4784D0 -> sub_42FC70 changes auxiliary-window visibility." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_DEBUG_WINDOW_TITLE, symbol: "UserB0_15_SetDebugWindowTitle", parameters: USER_SET_DEBUG_WINDOW_TITLE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478510 -> sub_42FD50 changes the auxiliary-window title." },
    NativeOpcodeSpec { opcode: opcodes::USER_MOVE_DEBUG_WINDOW, symbol: "UserB0_16_MoveDebugWindow", parameters: USER_MOVE_DEBUG_WINDOW_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478550 -> sub_42FCC0 moves one auxiliary window." },
    NativeOpcodeSpec { opcode: opcodes::USER_GET_DEBUG_WINDOW_POSITION, symbol: "UserB0_17_GetDebugWindowPosition", parameters: USER_GET_DEBUG_WINDOW_POSITION_PARAMETERS, returns: "x then y", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4785A0 -> sub_42FD00 pushes the auxiliary-window outer position as two stack outputs." },
    NativeOpcodeSpec { opcode: opcodes::USER_CLEAR_DEBUG_WINDOW, symbol: "UserB0_18_ClearDebugWindow", parameters: USER_CLEAR_DEBUG_WINDOW_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4785F0 -> sub_42FD80 fills the auxiliary-window backbuffer and repaints it." },
    NativeOpcodeSpec { opcode: opcodes::USER_DRAW_DEBUG_BITMAP, symbol: "UserB0_19_DrawDebugBitmap", parameters: USER_DRAW_DEBUG_BITMAP_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478630 -> sub_42FDE0 composites a graph bitmap into the auxiliary backbuffer." },
    NativeOpcodeSpec { opcode: opcodes::USER_DRAW_DEBUG_TEXT, symbol: "UserB0_1A_DrawDebugText", parameters: USER_DRAW_DEBUG_TEXT_PARAMETERS, returns: "rendered text extent", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478780 -> sub_42FEE0 rasterizes text into the auxiliary backbuffer and returns the updated extent." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_DEBUG_WINDOW_CLOSE_MESSAGE, symbol: "UserB0_1C_SetDebugWindowCloseMessage", parameters: USER_SET_DEBUG_WINDOW_CLOSE_MESSAGE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478920 -> sub_42FFD0 stores the auxiliary-window close message." },
    NativeOpcodeSpec { opcode: opcodes::USER_CREATE_EDIT_CONTROL, symbol: "UserB0_20_CreateEditControl", parameters: USER_CREATE_EDIT_CONTROL_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478960 -> sub_463F20 creates the singleton Win32 EDIT control and applies the stored initial text." },
    NativeOpcodeSpec { opcode: opcodes::USER_DESTROY_EDIT_CONTROL, symbol: "UserB0_21_DestroyEditControl", parameters: NO_PARAMETERS, returns: "boolean was active", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478AD0 -> sub_464190 destroys the singleton EDIT control." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_EDIT_FONT_SCALE, symbol: "UserB0_22_SetEditFontScale", parameters: USER_SET_EDIT_FONT_SCALE_PARAMETERS, returns: "boolean accepted", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478AF0 -> sub_4641F0 validates and applies the font scale." },
    NativeOpcodeSpec { opcode: opcodes::USER_QUERY_EDIT_ACTIVE, symbol: "UserB0_23_QueryEditActive", parameters: NO_PARAMETERS, returns: "boolean active", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478B20 -> sub_464280 returns the singleton edit-control active state." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_EDIT_VISIBLE, symbol: "UserB0_24_SetEditVisible", parameters: USER_SET_EDIT_VISIBLE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478B40 -> sub_464210 changes edit-control visibility." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_EDIT_COLOR, symbol: "UserB0_25_SetEditColor", parameters: USER_SET_EDIT_COLOR_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478B60 -> sub_464290 changes the edit-control text color." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_EDIT_TEXT, symbol: "UserB0_26_SetEditText", parameters: USER_SET_EDIT_TEXT_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478B80 -> sub_464300 stores and applies the edit-control text." },
    NativeOpcodeSpec { opcode: opcodes::USER_GET_EDIT_TEXT, symbol: "UserB0_27_GetEditText", parameters: USER_GET_EDIT_TEXT_PARAMETERS, returns: "bytes written", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478BA0 -> sub_464320 copies current edit text to caller memory; VM mediation owns the bounded write." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_EDIT_HIDE_ON_ENTER, symbol: "UserB0_28_SetEditHideOnEnter", parameters: USER_SET_EDIT_HIDE_ON_ENTER_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478BD0 -> sub_464350 sets the Enter-hide policy used by the edit WndProc." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_EDIT_PRINTABLE_INPUT, symbol: "UserB0_29_SetEditPrintableInput", parameters: USER_SET_EDIT_PRINTABLE_INPUT_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478BF0 -> sub_464360 sets the printable-input gate used by the edit WndProc." },
    NativeOpcodeSpec { opcode: opcodes::USER_SHOW_MESSAGE, symbol: "UserB0_80_ShowMessage", parameters: USER_SHOW_MESSAGE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478C10 opens an informational MB_OK message box using the configured title. The portable host uses native macOS, Windows, or Linux dialog bridges and preserves the configured title." },
    NativeOpcodeSpec { opcode: opcodes::USER_SHOW_YES_NO_MESSAGE, symbol: "UserB0_81_ShowYesNoMessage", parameters: USER_SHOW_YES_NO_MESSAGE_PARAMETERS, returns: "boolean yes", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478C40 opens MB_YESNO|MB_ICONQUESTION and returns whether IDYES was selected. The portable host maps its native Yes/No response to the same Boolean contract." },
    NativeOpcodeSpec { opcode: opcodes::USER_SHOW_TYPED_MESSAGE, symbol: "UserB0_82_ShowTypedMessage", parameters: USER_SHOW_TYPED_MESSAGE_PARAMETERS, returns: "boolean accepted", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478CA0 selects one of two native MessageBoxA modes and maps the button result to Boolean. The portable host maps typed Yes/No or OK/Cancel dialogs to the same result contract." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_MESSAGE_TITLE, symbol: "UserB0_83_SetMessageTitle", parameters: USER_SET_MESSAGE_TITLE_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478D50 -> sub_46BC30 stores the process-global message-box title." },
    NativeOpcodeSpec { opcode: opcodes::USER_SHOW_INPUT_DIALOG, symbol: "UserB0_84_ShowInputDialog", parameters: USER_SHOW_INPUT_DIALOG_PARAMETERS, returns: "dialog result", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478D70 passes a signed length, initial text, title and mutable output buffer to sub_45CB10. VM mediation preserves the signed-decimal filter and bounded Shift-JIS write." },
    NativeOpcodeSpec { opcode: opcodes::USER_SHOW_MULTI_FIELD_DIALOG, symbol: "UserB0_85_ShowMultiFieldDialog", parameters: USER_SHOW_MULTI_FIELD_DIALOG_PARAMETERS, returns: "dialog result", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478DC0 maps two output buffers, labels, initial values and independent limits into sub_45CEC0. VM mediation owns both bounded writes." },
    NativeOpcodeSpec { opcode: opcodes::USER_SHOW_SELECTION_DIALOG, symbol: "UserB0_86_ShowSelectionDialog", parameters: USER_SHOW_SELECTION_DIALOG_PARAMETERS, returns: "dialog result", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478E60 invokes the four-segment editor; the signed limit selects numeric versus alphanumeric filtering and accepted segments are joined with hyphens." },
    NativeOpcodeSpec { opcode: opcodes::USER_SHOW_EXTENDED_DIALOG, symbol: "UserB0_87_ShowExtendedDialog", parameters: USER_SHOW_EXTENDED_DIALOG_PARAMETERS, returns: "dialog result", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478EC0 selects the extended template and supplies two output buffers with independent limits and numeric filters." },
    NativeOpcodeSpec { opcode: opcodes::USER_SHOW_PATH_DIALOG, symbol: "UserB0_8C_ShowPathDialog", parameters: USER_SHOW_PATH_DIALOG_PARAMETERS, returns: "dialog result", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478F80 parses a newline-delimited option list and writes the selected row to caller memory." },
    NativeOpcodeSpec { opcode: opcodes::USER_SHOW_SIX_FIELD_DIALOG, symbol: "UserB0_8F_ShowSixFieldDialog", parameters: USER_SHOW_SIX_FIELD_DIALOG_PARAMETERS, returns: "dialog result", scheduling: NativeSchedulingEffect::Continue, notes: "sub_478FD0 edits four ten-byte text fields plus zero-based month/day selections and writes all six caller-owned values on acceptance." },
    NativeOpcodeSpec { opcode: opcodes::USER_CREATE_MODELESS_DIALOG, symbol: "UserB0_A0_CreateModelessDialog", parameters: USER_CREATE_MODELESS_DIALOG_PARAMETERS, returns: "boolean success", scheduling: NativeSchedulingEffect::Continue, notes: "sub_479040 passes nine initial control values to sub_45E310, accepts only mode zero and writes the allocated handle. Target sub_45E310 creates a separate modeless Win32 dialog HWND with CreateDialogParamA. The portable host currently mirrors its five 0..128 sliders, four binary controls and queued event pairs in-window, but that compatibility surface must not globally capture unrelated main-window input." },
    NativeOpcodeSpec { opcode: opcodes::USER_CLOSE_MODELESS_DIALOG, symbol: "UserB0_A1_CloseModelessDialog", parameters: USER_CLOSE_MODELESS_DIALOG_PARAMETERS, returns: "boolean success", scheduling: NativeSchedulingEffect::Continue, notes: "sub_479090 -> sub_45E3B0 closes one modeless dialog and releases its queued host state." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_MODELESS_DIALOG_VISIBLE, symbol: "UserB0_A2_SetModelessDialogVisible", parameters: USER_SET_MODELESS_DIALOG_VISIBLE_PARAMETERS, returns: "boolean success", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4790C0 -> sub_45E420 toggles the separate dialog HWND with ShowWindow. In the portable in-window compatibility representation, only coordinates inside the overlay bounds are consumed; unrelated main-window clicks remain game input." },
    NativeOpcodeSpec { opcode: opcodes::USER_POLL_MODELESS_DIALOG, symbol: "UserB0_A3_PollModelessDialog", parameters: USER_POLL_MODELESS_DIALOG_PARAMETERS, returns: "0 event, 1 empty, -1 invalid, -2 error", scheduling: NativeSchedulingEffect::Continue, notes: "sub_479100 -> sub_45DE70 drains one [control,value] event pair; VM mediation owns the exact two-DWORD write and 0/1/-1 status mapping." },
    NativeOpcodeSpec { opcode: opcodes::USER_INTERN_FONT_NAME, symbol: "UserB0_C0_InternFontName", parameters: USER_INTERN_FONT_NAME_PARAMETERS, returns: "stable font id", scheduling: NativeSchedulingEffect::Continue, notes: "sub_479190 -> sub_468A70 interns one font name without metadata registration." },
    NativeOpcodeSpec { opcode: opcodes::USER_INTERN_FONT_NAME_WITH_OPTION, symbol: "UserB0_C1_InternFontNameWithOption", parameters: USER_INTERN_FONT_NAME_WITH_OPTION_PARAMETERS, returns: "stable font id", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4791C0 -> sub_468A70 interns one font name with an explicit option." },
    NativeOpcodeSpec { opcode: opcodes::USER_REGISTER_FONT_RESOURCE, symbol: "UserB0_C2_RegisterFontResource", parameters: USER_REGISTER_FONT_RESOURCE_PARAMETERS, returns: "registration status", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4791F0 -> sub_468BD0 registers a filesystem font resource." },
    NativeOpcodeSpec { opcode: opcodes::USER_REGISTER_ARCHIVE_FONT_RESOURCE, symbol: "UserB0_C3_RegisterArchiveFontResource", parameters: USER_REGISTER_ARCHIVE_FONT_RESOURCE_PARAMETERS, returns: "registration status", scheduling: NativeSchedulingEffect::Continue, notes: "sub_479220 registers an archive/memory font through the target font-resource helper." },
    NativeOpcodeSpec { opcode: opcodes::USER_QUERY_FONT_AVAILABLE, symbol: "UserB0_C4_QueryFontAvailable", parameters: USER_QUERY_FONT_AVAILABLE_PARAMETERS, returns: "availability/count", scheduling: NativeSchedulingEffect::Continue, notes: "sub_479260 -> sub_468F50 enumerates the requested font family." },
    NativeOpcodeSpec { opcode: opcodes::USER_QUERY_FONT_CAPABILITY, symbol: "UserB0_C6_QueryFontCapability", parameters: USER_QUERY_FONT_CAPABILITY_PARAMETERS, returns: "boolean capability", scheduling: NativeSchedulingEffect::Continue, notes: "sub_479290 -> sub_468F70 compares two target font records/capabilities." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_FONT_ALIAS, symbol: "UserB0_C7_SetFontAlias", parameters: USER_SET_FONT_ALIAS_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4792C0 -> sub_468F60 installs a target font alias/default mapping." },
    NativeOpcodeSpec { opcode: opcodes::USER_SET_DESKTOP_WALLPAPER, symbol: "UserB0_F0_SetDesktopWallpaper", parameters: USER_SET_DESKTOP_WALLPAPER_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4792F0 -> sub_498D30 updates wallpaper style/tile fields and applies the image. The portable host uses platform-native wallpaper services on macOS, Windows, and Linux." },


    // SystemC0 particle, rain, spline, and BWEF handlers.
    NativeOpcodeSpec { opcode: opcodes::USER2_CREATE_PARTICLE_SCREEN, symbol: "UserC0_00_CreateParticleScreen", parameters: USER_C0_00_PARAMETERS, returns: "particle-screen handle", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4755B0 -> sub_490CD0 -> sub_441130 allocates one of eight CDspObjPrtclScrn slots and returns a 0xC0000000-tagged handle." },
    NativeOpcodeSpec { opcode: opcodes::USER2_RELEASE_PARTICLE_SCREEN, symbol: "UserC0_01_ReleaseParticleScreen", parameters: USER_C0_01_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475650 -> sub_490CE0 -> sub_441280 releases one particle-screen slot." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_PARTICLE_SCREEN_ENABLED, symbol: "UserC0_04_SetParticleScreenEnabled", parameters: USER_C0_04_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475680 -> sub_490CF0 -> sub_441300 changes the particle display-object enable state." },
    NativeOpcodeSpec { opcode: opcodes::USER2_CONFIGURE_PARTICLE_SCREEN_DISPLAY, symbol: "UserC0_05_ConfigureParticleScreenDisplay", parameters: USER_C0_05_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4756C0 -> sub_490D00 -> sub_441370 configures the display transform and composition state." },
    NativeOpcodeSpec { opcode: opcodes::USER2_CONFIGURE_PARTICLE_FRAME_TABLES, symbol: "UserC0_06_ConfigureParticleFrameTables", parameters: USER_C0_06_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475750 -> sub_490D20 -> sub_4413E0/sub_4246E0 builds the particle frame tables; VM mediation owns both raw DWORD arrays." },
    NativeOpcodeSpec { opcode: opcodes::USER2_COMMIT_PARTICLE_SCREEN, symbol: "UserC0_08_CommitParticleScreen", parameters: USER_C0_08_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475800 -> sub_490D40 -> sub_441480 commits/resets the particle-screen native state." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_PARTICLE_AUTO_UPDATE, symbol: "UserC0_09_SetParticleAutoUpdate", parameters: USER_C0_09_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475830 -> sub_490C30 maintains the target automatic particle-update linked list." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_PARTICLE_CAPACITY, symbol: "UserC0_0A_SetParticleCapacity", parameters: USER_C0_0A_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475870 -> sub_490D50 -> sub_4414B0/sub_424A50 sets the validated particle-generator field." },
    NativeOpcodeSpec { opcode: opcodes::USER2_CONFIGURE_PARTICLE_EMITTER_TRANSFORM, symbol: "UserC0_0B_ConfigureParticleEmitterTransform", parameters: USER_C0_0B_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4758B0 -> sub_490D60 -> sub_4414E0/sub_424A10 forwards the recovered ten-value emitter transform/layout contract." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_PARTICLE_EMISSION_PERCENT, symbol: "UserC0_0C_SetParticleEmissionPercent", parameters: USER_C0_0C_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4759C0 -> sub_490D90 -> sub_441540/sub_424A50 validates and applies an emission percentage." },
    NativeOpcodeSpec { opcode: opcodes::USER2_ADVANCE_PARTICLE_SCREEN, symbol: "UserC0_0D_AdvanceParticleScreen", parameters: USER_C0_0D_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475A40 -> sub_490DA0 -> sub_441580 advances the particle simulation." },
    NativeOpcodeSpec { opcode: opcodes::USER2_RESET_PARTICLE_SCREEN, symbol: "UserC0_0F_ResetParticleScreen", parameters: USER_C0_0F_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475A80 -> sub_490DB0 -> sub_4415B0 resets the particle generator." },
    NativeOpcodeSpec { opcode: opcodes::USER2_CONFIGURE_PARTICLE_EMITTER, symbol: "UserC0_10_ConfigureParticleEmitter", parameters: USER_C0_10_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475AB0 -> sub_490DC0 -> sub_4415E0/sub_424A70 configures the target particle emitter with eleven fields plus handle." },
    NativeOpcodeSpec { opcode: opcodes::USER2_CONFIGURE_PARTICLE_ANIMATION_BANK, symbol: "UserC0_18_ConfigureParticleAnimationBank", parameters: USER_C0_18_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475B80 configures the target particle animation/resource bank and preserves its positional ABI." },
    NativeOpcodeSpec { opcode: opcodes::USER2_LOAD_PARTICLE_ANIMATION_FRAMES, symbol: "UserC0_1A_LoadParticleAnimationFrames", parameters: USER_C0_1A_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475CD0 loads a particle animation frame range/resource configuration." },
    NativeOpcodeSpec { opcode: opcodes::USER2_COMMIT_PARTICLE_ANIMATION_FRAMES, symbol: "UserC0_1B_CommitParticleAnimationFrames", parameters: USER_C0_1B_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475E40 commits one particle animation frame configuration." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_PARTICLE_INTERPOLATION_MODE, symbol: "UserC0_1F_SetParticleInterpolationMode", parameters: USER_C0_1F_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475F20 -> sub_4910E0 accepts exactly interpolation modes 0, 1, and 2." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_PARTICLE_STYLE0, symbol: "UserC0_20_SetParticleStyle0", parameters: USER_C0_20_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_475F80 -> sub_491100/sub_441640 configures one target particle style path and maps target status codes." },
    NativeOpcodeSpec { opcode: opcodes::USER2_DEFINE_PARTICLE_STYLE0, symbol: "UserC0_24_DefineParticleStyle0", parameters: USER_C0_24_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476040 defines one particle style/resource selector." },
    NativeOpcodeSpec { opcode: opcodes::USER2_CONFIGURE_PARTICLE_STYLE0, symbol: "UserC0_25_ConfigureParticleStyle0", parameters: USER_C0_25_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476110 dereferences the top target pointer and configures the first extended particle style." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_PARTICLE_STYLE1, symbol: "UserC0_28_SetParticleStyle1", parameters: USER_C0_28_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476240 -> sub_491160/sub_4416A0 configures the second target particle style path." },
    NativeOpcodeSpec { opcode: opcodes::USER2_CONFIGURE_PARTICLE_STYLE1, symbol: "UserC0_29_ConfigureParticleStyle1", parameters: USER_C0_29_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476300 -> sub_491180/sub_441700 forwards the recovered sixteen-value style contract." },
    NativeOpcodeSpec { opcode: opcodes::USER2_DEFINE_PARTICLE_STYLE1, symbol: "UserC0_2C_DefineParticleStyle1", parameters: USER_C0_2C_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476490 defines the second particle style/resource selector." },
    NativeOpcodeSpec { opcode: opcodes::USER2_CONFIGURE_PARTICLE_ADVANCED_STYLE, symbol: "UserC0_2D_ConfigureParticleAdvancedStyle", parameters: USER_C0_2D_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476560 dereferences its fourth native pop and forwards the eighteen-value advanced particle style contract." },
    NativeOpcodeSpec { opcode: opcodes::USER2_CREATE_RAIN_SCREEN, symbol: "UserC0_40_CreateRainScreen", parameters: USER_C0_40_PARAMETERS, returns: "rain-screen handle", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476720 -> sub_492150 -> sub_4418B0 allocates one of eight CDspObjRainScrn slots and returns a 0xC1000000-tagged handle." },
    NativeOpcodeSpec { opcode: opcodes::USER2_RELEASE_RAIN_SCREEN, symbol: "UserC0_41_ReleaseRainScreen", parameters: USER_C0_41_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4767C0 -> sub_492160 -> sub_441A00 releases one rain-screen slot." },
    NativeOpcodeSpec { opcode: opcodes::USER2_INITIALIZE_RAIN_SCREEN, symbol: "UserC0_42_InitializeRainScreen", parameters: USER_C0_42_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4767F0 -> sub_492170 -> sub_441A80/sub_424F80 creates and initializes the native rain generator." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_RAIN_TEXTURE, symbol: "UserC0_43_SetRainTexture", parameters: USER_C0_43_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476830 -> sub_492180 -> sub_441AB0/sub_425620 validates and assigns a type-3 graph resource." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_RAIN_ENABLED, symbol: "UserC0_44_SetRainEnabled", parameters: USER_C0_44_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4768D0 -> sub_492190 -> sub_441B30 changes the rain display-object enable state." },
    NativeOpcodeSpec { opcode: opcodes::USER2_CONFIGURE_RAIN_DISPLAY, symbol: "UserC0_45_ConfigureRainDisplay", parameters: USER_C0_45_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476910 -> sub_4921A0 -> sub_441BA0 configures the rain display transform and composition state." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_RAIN_VOLUME_BOUNDS, symbol: "UserC0_46_SetRainVolumeBounds", parameters: USER_C0_46_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_4769B0 -> sub_4921C0 -> sub_441C20/sub_425160 updates CDspObjRainScrn fields 77..82." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_RAIN_DROP_WIDTH, symbol: "UserC0_47_SetRainDropWidth", parameters: USER_C0_47_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476A40 -> sub_4921E0 -> sub_441CB0/sub_4251D0 stores width << 8." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_RAIN_DROP_HEIGHT, symbol: "UserC0_48_SetRainDropHeight", parameters: USER_C0_48_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476AC0 -> sub_4921F0 -> sub_441D30/sub_425220 stores height << 8." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_RAIN_COLOR, symbol: "UserC0_49_SetRainColor", parameters: USER_C0_49_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476B40 -> sub_492200 -> sub_441DB0/sub_425270 updates CDspObjRainScrn field 88." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_RAIN_DENSITY, symbol: "UserC0_4A_SetRainDensity", parameters: USER_C0_4A_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476B80 -> sub_492210 -> sub_441E30/sub_4252C0 updates CDspObjRainScrn field 90." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_RAIN_SPEED, symbol: "UserC0_4B_SetRainSpeed", parameters: USER_C0_4B_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476BC0 -> sub_492220 -> sub_441EB0/sub_425310 updates CDspObjRainScrn field 91." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_RAIN_ORIGIN, symbol: "UserC0_4C_SetRainOrigin", parameters: USER_C0_4C_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476C40 -> sub_492230 -> sub_441F30/sub_425360 updates CDspObjRainScrn fields 93..95." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_RAIN_DIRECTION, symbol: "UserC0_4D_SetRainDirection", parameters: USER_C0_4D_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476CA0 -> sub_492250 -> sub_441FB0/sub_4253B0 updates CDspObjRainScrn fields 96..98." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_RAIN_LENGTH, symbol: "UserC0_4E_SetRainLength", parameters: USER_C0_4E_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476D00 -> sub_492270 -> sub_442030/sub_425400 updates CDspObjRainScrn field 99." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SET_RAIN_STEP, symbol: "UserC0_4F_SetRainStep", parameters: USER_C0_4F_PARAMETERS, returns: "void", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476D80 -> sub_492280 resets the global rain timer, stores the update interval, applies the native step, and wakes the renderer." },
    NativeOpcodeSpec { opcode: opcodes::USER2_CREATE_SPLINE, symbol: "UserC0_C0_CreateSpline", parameters: NO_PARAMETERS, returns: "spline handle", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476DF0 -> sub_494930 allocates a stable CSpline registry handle." },
    NativeOpcodeSpec { opcode: opcodes::USER2_RELEASE_SPLINE, symbol: "UserC0_C1_ReleaseSpline", parameters: USER_C0_C1_PARAMETERS, returns: "0 success, 1 missing", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476E10 -> sub_4949E0 removes one CSpline registry entry." },
    NativeOpcodeSpec { opcode: opcodes::USER2_CONFIGURE_SPLINE, symbol: "UserC0_C2_ConfigureSpline", parameters: USER_C0_C2_PARAMETERS, returns: "0 success, 1 missing, 2 count, 3 duration", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476E40 -> sub_494A60 builds independent natural cubic splines for x/y/z; VM mediation reads 16-byte records." },
    NativeOpcodeSpec { opcode: opcodes::USER2_SAMPLE_SPLINE, symbol: "UserC0_C3_SampleSpline", parameters: USER_C0_C3_PARAMETERS, returns: "0 success, 1 missing, 4 range, -1 failure", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476E90 -> sub_494AE0 samples the natural cubic spline and writes one XYZ triple through VM memory." },
    NativeOpcodeSpec { opcode: opcodes::USER2_LOAD_BWEF_TABLE, symbol: "UserC0_F0_LoadBwefTable", parameters: USER_C0_F0_PARAMETERS, returns: "target BWEF status", scheduling: NativeSchedulingEffect::Continue, notes: "sub_476ED0 -> sub_4066C0 validates the exact bwef magic/length and writes the table and count; VM mediation owns caller memory." },
